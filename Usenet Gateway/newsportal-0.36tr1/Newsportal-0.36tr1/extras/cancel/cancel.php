  ...
  if ($type=="cancel") {
    if (testGroupAccess($group)) {
      ...
      $message=message_cancel(quoted_printable_encode(stripslashes($subject)),
                 quoted_printable_encode(stripslashes($name))." <".$email.">",
                 $newsgroups,explode(" ",$references),$body,$format);
      // Article sent without errors?
      if (substr($message,0,3)=="240") {
?>

<h1 align="center">Cancel an article</h1>
...
if ($testnewsgroups == "") {
  ...
  echo '<h1 align="center">Article in '.$newsgroups
    .' canceled</h1>';

echo '<p><b>Warning!</b> Der Artikel wird auf allen <b>allen</b> '.
...
<textarea name="body" rows="10" cols="79" wrap="virtual">
