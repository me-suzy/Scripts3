  ...
  if ($content_type[0]=="multipart") {
    ...
    for($i++;$i<$count_rawmessage; $i++) {
      ...
      // yes, it seems, we have!
        ...
        $uue_data="";
        $i++;
        while($rawmessage[$i]!="end" && $rawmessage[$i]!="") {
        $backupdata.=$rawmessage[$i]."\n";
          if (strlen(trim($rawmessage[$i])) > 2)
            $uue_data.=$rawmessage[$i]."\n";
          $i++;
        }
        if($rawmessage[$i]==""){
          $body .= $uue_infoline_raw."\n";
          $body .= $backupdata;
          $noattachment=1;
        }
        if(!$noattachment){
...
function show_header($head,$group) {
  ...
  global $file_attachment,$anonym_address,$show_xface,$file_xface;
  echo '<div class="np_article_header">';
  if ($show_xface!=false && $head->xface != "") echo '<img width="48" height="48" '.
    'src="'.$file_xface.'&amp;xface='.urlencode($head->xface).'" alt="xface preview" '.
    "style=\"float: right\" />\n";
  ...
  if ($article_show["From"]) {
    ...
    if($head->from==$anonym_address) {
      ...
      if($article_show["From_link"])
        echo'<a href="mailto:'.eregi_replace($article_show["From_rewrite"][0],
          $article_show["From_rewrite"][1],htmlspecialchars($head->from)).'">';
      echo str_replace($article_show["From_rewrite"][0],
          $article_show["From_rewrite"][1],htmlspecialchars($head->from));
      if($article_show["From_link"])
        echo '</a>';
      echo '&gt;';
      ...
      for ($i=0; $i<=count($body) && 
        ...
        if($article_graphicquotes) {
          ...
          for($j=$body[$i]->depth; $j<$depth; $j++)
            echo "</blockquote>\n";
          echo ' <tt class="np_article_text">';
          $t=text2html($body[$i]->text)."</tt><br />\n";
          echo $t;
          $currentlen+=strlen($t);
          echo "\n";
          $depth=$body[$i]->depth;
          ...
          if($body[$i]->depth==0) {
            if(trim($body[$i]->text)=='')
              ...
              $t=' <tt class="np_article_text">'.text2html($body[$i]->text).
                   "</tt><br />\n";
          } else {
            $t=' <tt class="np_article_text"><i>'.str_repeat('&gt;',
               $body[$i]->depth).' '.text2html(textwrap($body[$i]->text,
               80-$body[$i]->depth, "<br />\n".str_repeat('&gt;',
               $body[$i]->depth).' '))."</i></tt><br />\n";
          }
          echo $t;
          $currentlen+=strlen($t);
        }
