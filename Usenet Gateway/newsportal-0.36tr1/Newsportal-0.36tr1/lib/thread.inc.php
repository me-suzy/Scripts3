...
function thread_load($groupname,$readmode = 1,$poll=false) {
  ...
  if($query_ns) {
    ...
    if ((isset($articles)) && ($articles)) {
      if (($thread_sort_order != 0) && (count($articles)>0))
        uasort($articles,'thread_mycompare');
...
function thread_format_author($c) {
  ...
  if($thread_show["authorlink"])
    $return .= '<a href="mailto:'.str_replace('@','&#0064;',trim($c->from)).'">';
  if (trim($c->name)!="") { 
...
function thread_show_head() {
  ...
  if (($thread_treestyle==2) || ($thread_treestyle==6) ||
    ...
    if ($thread_show["date"])
      echo '<td style="width: 1%" class="np_thread_head">'.$text_thread["date"]."&nbsp;</td>";
    if ($thread_show["subject"])
