<?
/*  TRanx rebuilded Newsportal NNTP<->HTTP Gateway
 *  Version: 0.36 TRanx rebuild 1
 *  Download: http://www.newsportal.one.pl
 *
 *  Copyright (C) 2002-2004 Florian Amrhein
 *  E-Mail: florian.amrhein@gmx.de
 *  Web: http://florian-amrhein.de
 *  Changes: Adam Glowienka <tranx@newsportal.one.pl>
 *
...
function nntp_open($nserver=0,$nport=0) {
  global $text_error,$server_auth_user,$server_auth_pass;
  global $server,$port;
...
/*
 * Test, if access to post to a group is allowed.
 *
 * $groupname: name of the group to be checked
 *
 * returns true, if access to post is allowed
 */
function testGroupAccess($groupname) {
  global $readonly, $file_groupaccess;
  if(!$readonly)
    $allowwrite=true;
  if($gf=@fopen($file_groupaccess,"r")){
    while(!feof($gf)) {
      $read=trim(line_read($gf));
      $pos=strpos($read," ");
      if($pos != false && substr($read,0,$pos)==trim($groupname)) {
        if(substr($read,$pos+1, $pos+2)=="y")
          $allowwrite=true;
        elseif(substr($read,$pos+1, $pos+2)=="n")
          $allowwrite=false;
      }
    }
    fclose($gf);
...
function groups_read($server,$port) {
  global $gl_age,$file_groups,$spooldir,$cache_index,$text_index;
    ...
    while (!feof($gf)) {
      ...
      if(substr($tmp,0,1)==":") {
        ...
        if ($pos != false) {
          ...
          // no, try to get it from the newsserver.
          $gruppe->name=$tmp;
        ...
        $gruppe->count=$t[1];
        if($t[0]!=211)
          $gruppe->count=$text_index["newsgroup_not_avaiable"];
...
//        echo 'target="'.$frame_threadframeset.'" ';
      ...
      echo '<span class="np_thread_small">(';
      if($datecolor!="")
        echo '<span style="color: '.$datecolor.';">'.$g->count.'</span>';
      else
        echo $g->count;
      echo ')</span>';
      if($g->description!="-")
        echo '<br /><span class="np_thread_small">'.$g->description.'</span>';
      echo '</div>';
    }
    echo "\n";
    flush();
  ...
  for ($count=0;$count<count($hdr);$count++) {
    ...
    $value=trim(substr($hdr[$count],strpos($hdr[$count]," ")+1));
      switch (strtolower($variable)) {
        ...
        case "x-mailer:":
        case "user-agent:":
          $header->user_agent=headerDecode(trim($value));
          break;
        case "x-face:": // uses external converter
          $header->xface=trim($value);
          break;
        case "x-no-archive:":
...
function html_parse($text) {
  global $frame_externallink, $group, $file_article;
...
//  }
  ...
  $regexp_replace=array(
    ...
    'news:([-a-z0-9_.%$]+)@((\.*([-a-z0-9_])+)+)' =>
      '<a href="'.$file_article.'?group='.$group.'&amp;id=<%MID%>">news:\1@\2</a>',
    'mid:([-a-z0-9_.%$]+)@((\.*([-a-z0-9_])+)+)' =>
      '<a href="'.$file_article.'?group='.$group.'&amp;id=<%MID%>">mid:\1@\2</a>',
    '([-a-z0-9_.]+)@((\.*([-a-z0-9_])+)+)' =>
