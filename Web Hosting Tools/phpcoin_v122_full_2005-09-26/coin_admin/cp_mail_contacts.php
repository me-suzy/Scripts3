...
/**************************************************************
 * File: 		Control Panel: Mail Contacts
...
function cp_do_select_form_mail_contacts($aaction, $aname, $avalue, $aret_flag=0)
	{
		...
		# Set Query for select.
			$query	= "SELECT mc_id, mc_name FROM ".$_DBCFG['mail_contacts']." ORDER BY mc_id ASC";
		...
		# Build form output
			...
			# Process query results
				while(list($mc_id, $mc_name) = db_fetch_row($result))
				{
					$_out .= '<option value="'.$mc_id.'">'.str_pad($mc_id,3,'0',STR_PAD_LEFT).' - '.$mc_name.'</option>'.$_nl;
				}
...
function cp_do_form_add_edit_mail_contacts( $adata, $aerr_entry, $aret_flag=0)
	{
		...
		# Do Main Form
			$_cstr .= '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=mail_contacts&op='.$adata['op'].'">'.$_nl;
			...
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="mc_name" SIZE=30 value="'.$adata[mc_name].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l06_Email'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="mc_email" SIZE=30 value="'.$adata[mc_email].'" maxlength="50">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
...
IF ( $_GPV[op]=='none' )
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			# Call function for create select form: Mail Contact
				$aaction = $_SERVER["PHP_SELF"].'?cp=mail_contacts&op=edit';
				$aname	= "mc_id";
				$avalue	= $_GPV[mc_id];
				$_cstr .= cp_do_select_form_mail_contacts($aaction, $aname, $avalue, '1');
...
IF ($_GPV[op]=='add' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do select
			...
			$query		.= " VALUES ( '$_GPV[mc_id]', '$_GPV[mc_name]', '$_GPV[mc_email]', '$_GPV[mc_status]')";
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Add_Mail_Contacts_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')';
...
IF ($_GPV[op]=='edit' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do select
			...
			$query	.= "mc_name = '$_GPV[mc_name]', mc_email = '$_GPV[mc_email]', mc_status = '$_GPV[mc_status]'";
			$query	.= " WHERE mc_id = $_GPV[mc_id]";
			$result	= db_query_execute($query) OR DIE("Unable to complete request");
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Edit_Mail_Contacts_Entry_Results'];
			$_out .= do_subj_block_it ($title_text, '1');
...
IF ($_GPV[op]=='delete' && $_GPV[stage]==1)
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			$_cstr = '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=mail_contacts&op=delete">'.$_nl;
			$_cstr .= '<table cellpadding="5" width="100%">'.$_nl;
