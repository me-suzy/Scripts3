...
/**************************************************************
 * File: 		Control Panel: WHOIS Lookups
...
function cp_do_select_form_whois($aaction, $aname, $avalue, $aret_flag=0) {
	...
	while(list($whois_id, $whois_name) = db_fetch_row($result)) {
...
function cp_do_form_add_edit_whois($adata, $aerr_entry, $aret_flag=0) {
	...
	# Do Main Form
	$_cstr .= '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=whois&op='.$adata['op'].'">'.$_nl;
	...
	$_cstr .= $_td_str_right.$_nl;
	$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="whois_display" SIZE=25 value="'.$adata[whois_display].'">'.$_nl;
	$_cstr .= '</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
	...
	$_cstr .= $_td_str_right.$_nl;
	$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="whois_value" SIZE=20 value="'.$adata[whois_value].'">'.$_nl;
	$_cstr .= '</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
	...
	$_cstr .= $_td_str_right.$_nl;
	$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="whois_server" SIZE=40 value="'.$adata[whois_server].'" maxlength="100">'.$_nl;
	$_cstr .= '</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
	$_cstr .= '<tr>'.$_nl;
	$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l16_NoMatch_Return_Param'].$_sp.'</b></td>'.$_nl;
	$_cstr .= $_td_str_right.$_nl;
	$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="whois_nomatch" SIZE=20 value="'.$adata[whois_nomatch].'" maxlength="30">'.$_nl;
	$_cstr .= '</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
	...
	$_cstr .= $_td_str_right.$_nl;
	// Hide "0" to avoid confusion
	IF (!$adata[whois_prod_id]) {$adata[whois_prod_id] = '';}
	$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="whois_prod_id" SIZE=10 value="'.$adata[whois_prod_id].'" maxlength="30">'.$_nl;
	$_cstr .= '</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
	...
	$_cstr .= $_td_str_right.$_nl;
	$_cstr .= '<TEXTAREA class="PSML_NL" NAME="whois_notes" COLS=60 ROWS=15>'.do_stripslashes($adata[whois_notes]).'</TEXTAREA>'.$_nl;
	$_cstr .= '</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
...
function cp_do_display_entry_whois ($adata, $aret_flag=0) {
	...
	# Format entry
	$mod_notes = nl2br($adata[whois_notes]);
	...
	$_cstr .= '<tr valign="bottom">'.$_nl;
	$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l16_Override_Product'].$_sp.'</b></td>'.$_nl;
	$_cstr .= $_td_str_right.$adata[whois_prod_id].'</td>'.$_nl;
	$_cstr .= '</tr>'.$_nl;
...
IF ( $_GPV[op]=='view' ) {
	...
	IF ( $_GPV[whois_id] ) {
		...
		$query		.= " ORDER BY whois_display ASC";
...
IF ( $_GPV[op]=='none' ) {
	...
	$aaction = $_SERVER["PHP_SELF"].'?cp=whois&op=edit';
	$aname	= "whois_id";
	$avalue	= $_GPV[whois_id];
	$_cstr .= cp_do_select_form_whois($aaction, $aname, $avalue, '1').$_nl;
...
IF ($_GPV[op]=='add' && $_GPV[stage]==1 && !$err_entry['flag'])	{
	...
	$_GPV[whois_notes]	= do_addslashes($_GPV[whois_notes]);
	...
	$query		.= "whois_notes";
	$query		.= ") VALUES (";
	$query		.= "'$_GPV[whois_server]','$_GPV[whois_nomatch]','$_GPV[whois_value]',";
	$query		.= "'$_GPV[whois_display]','$_GPV[whois_include]','$_GPV[whois_prod_id]',";
	$query		.= "'$_GPV[whois_notes]'";
	$query		.= ")";
	...
	$title_text = $_LANG['_ADMIN']['Add_WHOIS_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')';
...
IF ($_GPV[op]=='edit' && (!$_GPV[stage] || $err_entry['flag'])) {
	...
	IF ( $_GPV[stage] == 1 && $err_entry['flag'] ) {
		...
		IF ( $_GPV[whois_id] ) {
			...
			$aname	= "whois_id";
			$avalue	= $_GPV[whois_id];
			$_cstr .= cp_do_select_form_whois($aaction, $aname, $avalue, '1');
			$_mstr .= do_nav_link ($_SERVER["PHP_SELF"], $_TCFG['_IMG_ADMIN_M'],$_TCFG['_IMG_ADMIN_M_MO'],'','');
			$_mstr .= do_nav_link ($_SERVER["PHP_SELF"].'?cp=whois&op=add', $_TCFG['_IMG_ADD_NEW_M'],$_TCFG['_IMG_ADD_NEW_M_MO'],'','');
...
IF ($_GPV[op]=='edit' && $_GPV[stage]==1 && !$err_entry['flag']) {
	...
	$query .= "whois_server = '$_GPV[whois_server]', ";
	$query .= "whois_nomatch = '$_GPV[whois_nomatch]', ";
	$query .= "whois_value = '$_GPV[whois_value]', ";
	$query .= "whois_display = '$_GPV[whois_display]', ";
	$query .= "whois_include = '$_GPV[whois_include]', ";
	$query .= "whois_prod_id = '$_GPV[whois_prod_id]', ";
	$query .= "whois_notes = '$_GPV[whois_notes]'";
	$query .= " WHERE whois_id = $_GPV[whois_id]";
	$result = db_query_execute($query) OR DIE("Unable to complete request");
	...
	# Call function to open block
	$title_text = $_LANG['_ADMIN']['Edit_WHOIS_Entry_Results'];
	$_out .= do_subj_block_it ($title_text, '1');
...
IF ($_GPV[op]=='delete' && $_GPV[stage]==1) {
	...
	$_cstr = '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=whois&op=delete">'.$_nl;
	$_cstr .= '<table cellpadding="5" width="100%">'.$_nl;
