...
function cp_do_select_form_cat($aaction, $aname, $avalue, $aret_flag=0)
	{
		...
		# Set Query for select.
			$query	= "SELECT cat_id, cat_name, cat_desc FROM ".$_DBCFG['categories']." ORDER BY cat_id ASC";
		...
		# Build form output
			...
			# Process query results
				while(list($cat_id, $cat_name, $cat_desc) = db_fetch_row($result))
				{
					$_out .= '<option value="'.$cat_id.'">'.str_pad($cat_id,3,'0',STR_PAD_LEFT).' - '.$cat_name.'</option>'.$_nl;
				}
...
function cp_do_form_add_edit_cat( $adata, $aerr_entry, $aret_flag=0)
	{
		...
		# Do Main Form
			$_cstr .= '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=categories&op='.$adata['op'].'">'.$_nl;
			...
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="cat_name" SIZE=30 value="'.$adata[cat_name].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l02_Description'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="cat_desc" SIZE=30 value="'.$adata[cat_desc].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
...
IF ( $_GPV[op]=='none' )
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			# Call function for create select form: Categories
				$aaction = $_SERVER["PHP_SELF"].'?cp=categories&op=edit';
				$aname	= "cat_id";
				$avalue	= $_GPV[cat_id];
				$_cstr .= cp_do_select_form_cat($aaction, $aname, $avalue, '1');
...
IF ($_GPV[op]=='add' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do select
			...
			$query		.= "  VALUES ( '$_GPV[cat_id]', '$_GPV[cat_pos]', '$_GPV[cat_name]', '$_GPV[cat_desc]', '$_GPV[cat_icon]')";
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Add_Categories_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')';
...
IF ($_GPV[op]=='edit' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do select
			$query	= "UPDATE ".$_DBCFG['categories']." SET cat_pos = '$_GPV[cat_pos]'";
			$query	.= ", cat_name = '$_GPV[cat_name]', cat_desc = '$_GPV[cat_desc]', cat_icon = '$_GPV[cat_icon]'";
			$query	.= " WHERE cat_id = $_GPV[cat_id]";
			$result	= db_query_execute($query) OR DIE("Unable to complete request");
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Edit_Categories_Entry_Results'];
			$_out .= do_subj_block_it ($title_text, '1');
...
IF ($_GPV[op]=='delete' && $_GPV[stage]==1)
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			$_cstr = '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=categories&op=delete">'.$_nl;
			$_cstr .= '<table cellpadding="5" width="100%">'.$_nl;
