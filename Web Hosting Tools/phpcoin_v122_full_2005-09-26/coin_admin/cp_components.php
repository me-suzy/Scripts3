...
function cp_do_select_form_comp($aaction, $aname, $avalue, $aret_flag=0)
	{
		...
		# Set Query for select.
			$query	= "SELECT comp_id, comp_name, comp_mod, comp_desc FROM ".$_DBCFG['components']." ORDER BY comp_id ASC";
		...
		# Build form output
			...
			# Process query results
				while(list($comp_id, $comp_name, $comp_mod, $comp_desc) = db_fetch_row($result))
				{
					$_out .= '<option value="'.$comp_id.'">'.str_pad($comp_id,3,'0',STR_PAD_LEFT).' - '.$comp_name.'-'.$comp_desc.'</option>'.$_nl;
				}
...
function cp_do_form_add_edit_comp( $adata, $aerr_entry )
	{
		...
		# Do Main Form
			$_cstr .= '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=components&op='.$adata['op'].'">'.$_nl;
			...
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="comp_type" SIZE=20 value="'.$adata[comp_type].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l04_Name'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="comp_name" SIZE=20 value="'.$adata[comp_name].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l04_Mod'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="comp_mod" SIZE=20 value="'.$adata[comp_mod].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l04_Description'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="comp_desc" SIZE=50 value="'.$adata[comp_desc].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l04_Page_Title'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="comp_ptitle" SIZE=50 value="'.$adata[comp_ptitle].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			$_cstr .= '<tr>'.$_nl;
...
IF ( $_GPV[op]=='none' )
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			# Call function for create select form: Components
				$aaction = $_SERVER["PHP_SELF"].'?cp=components&op=edit';
				$aname	= "comp_id";
				$avalue	= $_GPV[comp_id];
				$_cstr .= cp_do_select_form_comp($aaction, $aname, $avalue, '1');
...
IF ($_GPV[op]=='add' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do insert
			...
			$query 		.= ", comp_status";
			$query		.= ") VALUES (";
			$query		.= "'$_GPV[comp_type]','$_GPV[comp_name]','$_GPV[comp_mod]','$_GPV[comp_desc]'";
			$query		.= ",'$_GPV[comp_ptitle]','$_GPV[comp_col_num]','$_GPV[comp_isadmin]','$_GPV[comp_isuser]'";
			$query		.= ",'$_GPV[comp_status]'";
			$query		.= ")";
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Add_Components_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')';
...
IF ($_GPV[op]=='edit' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do select
			...
			$query	.= "comp_type = '$_GPV[comp_type]', comp_name = '$_GPV[comp_name]'";
			$query	.= ", comp_mod = '$_GPV[comp_mod]', comp_desc = '$_GPV[comp_desc]'";
			$query	.= ", comp_ptitle = '$_GPV[comp_ptitle]', comp_col_num = '$_GPV[comp_col_num]'";
			$query	.= ", comp_isadmin = '$_GPV[comp_isadmin]', comp_isuser = '$_GPV[comp_isuser]'";
			$query	.= ", comp_status = '$_GPV[comp_status]'";
			$query	.= " WHERE comp_id = $_GPV[comp_id]";
			$result = db_query_execute($query) OR DIE("Unable to complete request");
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Edit_Components_Entry_Results'];
			$_out .= do_subj_block_it ($title_text, '1');
...
IF ($_GPV[op]=='delete' && $_GPV[stage]==1)
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			$_cstr = '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=components&op=delete">'.$_nl;
			$_cstr .= '<table cellpadding="5" width="100%">'.$_nl;
