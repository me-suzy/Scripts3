...
/**************************************************************
 * File: 		Control Panel: Downloads
 * Author:	Stephen M. Kitching (http://phpCOIN.com)
 * Date:		2005-03-09 (v1.2.2)
 * License:	DO NOT Remove this text block. See /coin_docs/license.txt
 *			Copyright © 2003-2004-2005 COINSoft Technologies Inc.
...
# Check required fields (err / action generated later in cade as required)
	...
	# Call validate input function
		$err_entry = cp_do_input_validation($data);
	}
...
IF ( $_GPV['op']=='none' ) {
	...
	# Call function for create select form: Downloads
		$aaction = $_SERVER["PHP_SELF"].'?cp=downloads&op=edit';
		$_cstr = cp_do_select_form_downloads($aaction, 'dload_id', $_GPV['dload_id'], '1');
...
IF ($_GPV['op'] == 'add' && $_GPV['stage'] == 1 && !$err_entry['flag']) {
	...
	# Process inputs for quotes
		$_GPV['dload_desc']	= do_addslashes($_GPV['dload_desc']);

	# Do select
		$query  = "INSERT INTO ".$_DBCFG['downloads']." (";
		$query .= "dload_id, dload_group, dload_name, dload_desc, dload_count, dload_date_str, dload_avail, dload_filename, dl_size, dload_contributor";
		$query .= ") VALUES (";
		$query .= "'', '".$_GPV['dload_group']."', '".$_GPV['dload_name']."', '";
		$query .= $_GPV['dload_desc']."', '".$_GPV['dload_count']."', '".$_GPV['dload_date_str']."', '";
		$query .= $_GPV['dload_avail']."', '".$_GPV['dload_filename']."', '".$_GPV['dl_size']."', '";
		$query .= $_GPV['dload_contributor']."'";
		$query .= ")";
	...
	# Call function for Display Entry
		...
		$title_text = $_LANG['_ADMIN']['Add_Downloads_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')';
...
IF ($_GPV['op'] == 'edit' && (!$_GPV['stage'] || $err_entry['flag'])) {
	...
	# Check for valid $_GPV['dload_id'] no
		...
		# Call function for create select form: Downloads
			$aaction	 = $_SERVER["PHP_SELF"].'?cp=downloads&op=edit';
			$aname	 = "dload_id";
			$avalue	 = $_GPV['dload_id'];
			$_cstr	 = cp_do_form_add_edit_downloads($aaction, $aname, $avalue, '1');
...
IF ($_GPV['op'] == 'edit' && $_GPV['stage'] == 1 && !$err_entry['flag']) {
	...
	# Do select
		...
		$query .= "dload_name = '".$_GPV['dload_name']."', ";
		$query .= "dload_desc = '".$_GPV['dload_desc']."', ";
		$query .= "dload_group = '".$_GPV['dload_group']."', ";
		$query .= "dload_count = '".$_GPV['dload_count']."', ";
		$query .= "dload_date_str = '".$_GPV['dload_date_str']."', ";
		$query .= "dload_avail = '".$_GPV['dload_avail']."', ";
		$query .= "dload_filename = '".$_GPV['dload_filename']."', ";
		$query .= "dl_size = '".$_GPV['dl_size']."', ";
		$query .= "dload_contributor = '".$_GPV['dload_contributor']."' ";
		$query .= "WHERE dload_id = ".$_GPV['dload_id'];
		$result = db_query_execute($query) OR DIE("Unable to complete request");
...
IF ($_GPV['op'] == 'delete' && $_GPV['stage'] == 1) {
	...
	# Do confirmation form to content string
		$_cstr  = '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=downloads&op=delete">'.$_nl;
...
function cp_do_input_validation($adata) {
	...
	#	IF (!$adata['dload_avail'])		{ $err_entry['flag'] = 1; $err_entry['dload_avail'] = 1;		}
		...
		IF (!$adata['dl_size'])			{ $err_entry['flag'] = 1; $err_entry['dl_size'] = 1;		}
...
function cp_do_select_form_downloads($aaction, $aname, $avalue, $aret_flag=0) {
	...
	# Set Query for select.
		$query = "SELECT dload_id, dload_desc FROM ".$_DBCFG['downloads']." ORDER BY dload_id ASC";
	...
	# Process query results
		while(list($dload_id, $dload_desc) = db_fetch_row($result)) {
			$_out .= '<option value="'.$dload_id.'">'.str_pad($dload_id,3,'0',STR_PAD_LEFT).' - '.$dload_desc.'</option>'.$_nl;
		}
...
function cp_do_form_add_edit_downloads( $adata, $aerr_entry, $aret_flag=0) {
	...
	# Do data entry error string check and build
		IF ($aerr_entry['flag']) {
			...
			IF ($aerr_entry['dl_size']) 			{ IF ($err_prv) { $err_str .= ', '; } $err_str .= $_LANG['_ADMIN']['AD20_ERR_09']; $err_prv = 1; }
	...
	# Do Main Form
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dload_group" SIZE=50 value="'.$adata['dload_group'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_Name'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dload_name" SIZE=50 value="'.$adata['dload_name'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left_vtop.'<b>'.$_LANG['_ADMIN']['l20_Download_Description'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<TEXTAREA class="PSML_NL" NAME="dload_desc" COLS="60" ROWS="5">'.do_stripslashes($adata['dload_desc']).'</TEXTAREA>'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_Date'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dload_date_str" SIZE=50 value="'.$adata['dload_date_str'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_FileName'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dload_filename" SIZE=50 value="'.$adata['dload_filename'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_FileSize'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dl_size" SIZE=10 value="'.$adata['dl_size'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_Contributor'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dload_contributor" SIZE=50 value="'.$adata['dload_contributor'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_Count'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="dload_count" SIZE=10 value="'.$adata['dload_count'].'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '<tr>'.$_nl;
...
function cp_do_display_entry_downloads ( $adata, $aret_flag=0) {
	...
	# Build Title String, Content String, and Footer Menu String
		...
		$_cstr .= '<tr valign="bottom">'.$_nl;
		$_cstr .= $_td_str_left_vtop.'<b>'.$_LANG['_ADMIN']['l20_Download_Description'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.nl2br(do_stripslashes($adata['dload_desc'])).'</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= '<tr valign="bottom">'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_FileSize'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$adata['dl_size'].'</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		$_cstr .= '<tr valign="bottom">'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l20_Download_Contributor'].$_sp.'</b></td>'.$_nl;
