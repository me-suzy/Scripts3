...
function cp_do_select_form_banip($aaction, $aname, $avalue, $aret_flag=0)
	{
		...
		# Build form output
			...
			# Process query results
				while(list($banned_ip) = db_fetch_row($result))
				{
...
function cp_do_form_add_edit_banip( $adata, $aerr_entry, $aret_flag=0)
	{
		...
		# Do Main Form
			$_cstr .= '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=banip&op='.$adata['op'].'">'.$_nl;
			$_cstr .= '<table cellpadding="5" width="100%">'.$_nl;
			$_cstr .= '<tr>'.$_nl;
			$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l03_Banned_IP'].$_sp.'</b></td>'.$_nl;
			$_cstr .= $_td_str_right.$_nl;
			$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="banned_ip" SIZE=15 value="'.$adata[banned_ip].'">'.$_nl;
			$_cstr .= '</td>'.$_nl;
			$_cstr .= '</tr>'.$_nl;
			...
			IF ( $_GPV[banned_ip] )
				{
					# Set Query for select.
						...
						$query		.= " WHERE banned_ip='".$_GPV[banned_ip]."'";
...
IF ( $_GPV[op]=='none' )
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			# Call function for create select form: Banned IP
				$aaction = $_SERVER["PHP_SELF"].'?cp=banip&op=edit';
				$aname	= "banned_ip";
				$avalue	= $_GPV[banned_ip];
				$_cstr .= cp_do_select_form_banip($aaction, $aname, $avalue, '1');
...
IF ($_GPV[op]=='add' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		# Generate encrypted password
			$user_pwrd_encrypt = crypt($user_pwrd);

		# Dim some vars
			$query = ""; $result= ""; $numrows = 0;

		# Do insert
			$query 		= "INSERT INTO ".$_DBCFG['banned']." (banned_ip)";
			$query		.= " VALUES ( '$_GPV[banned_ip]')";
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Add_Banned_IP_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$_GPV[banned_ip].')';
...
IF ($_GPV[op]=='edit' && $_GPV[stage]==1 && !$err_entry['flag'])
	{
		...
		# Do select
			$query	= "UPDATE ".$_DBCFG['banned']." SET banned_ip = '$_GPV[banned_ip]'";
			$query	.= " WHERE banned_ip = '".$_GPV[banned_ip_orig]."'";
			$result = db_query_execute($query) OR DIE("Unable to complete request");
		...
		# Call function to open block
			$title_text = $_LANG['_ADMIN']['Edit_Banned_IP_Entry_Results'];
			$_out .= do_subj_block_it ($title_text, '1');
...
IF ($_GPV[op]=='delete' && $_GPV[stage]==1)
	{
		...
		# Build Title String, Content String, and Footer Menu String
			...
			$_cstr = '<FORM METHOD="POST" ACTION="'.$_SERVER["PHP_SELF"].'?cp=banip&op=delete">'.$_nl;
			$_cstr .= '<table cellpadding="5" width="100%">'.$_nl;
...
IF ($_GPV[op]=='delete' && $_GPV[stage]==2)
	{
		...
		# Do select
			$query		= "DELETE FROM ".$_DBCFG['banned']." WHERE banned_ip='".$_GPV[banned_ip]."'";
			$result		= db_query_execute($query) OR DIE("Unable to complete request");
