...
$did = $HTTP_COOKIE_VARS["oid"];
if ($did == "") {
echo '<title>Order Error</title><br>
Your 2Checkout transaction could NOT be processed, no authentication code was provided for authorization.';
} else {
...
SQLact("query", "INSERT INTO freelancers_transactions (amount, details, username, type, balance, date, date2) VALUES ('+" . SQLact("result", $result,0,"amount") . "', '2Checkout Deposit (" . SQLact("result", $result,0,"oid") . ")', '" . SQLact("result", $result,0,"username") . "', 'freelancer', '$newbal', '$month/$day/$year  $hours:$minutes', '" . time() . "')");
if ($depositnotify == "enabled") {
mail($emailaddress,$companyname . " 2Checkout Deposit (" . SQLact("result", $result,0,"oid") . ")","The " . SQLact("result", $result,0,"atype") . "\"" . SQLact("result", $result,0,"username") . "\" has just deposited " . $currencytype . "" . $currency . "" . SQLact("result", $result,0,"amount") . " into their account (Order Number: " . SQLact("result", $result,0,"oid") . ").","From: $emailaddress");
}
$today = getdate();
$month = $today['mon'];
$day = $today['mday'];
...
SQLact("query", "INSERT INTO freelancers_profits (amount, type, type2, month, year) VALUES ('$tram', 'deposit', 'cc', '" . date("n") . "', '" . date("Y") . "')");
...
if ($depositnotify == "enabled") {
mail($emailaddress,$companyname . " 2Checkout Deposit (" . SQLact("result", $result,0,"oid") . ")","The " . SQLact("result", $result,0,"atype") . "\"" . SQLact("result", $result,0,"username") . "\" has just deposited " . $currencytype . "" . $currency . "" . SQLact("result", $result,0,"amount") . " into their account (Order Number: " . SQLact("result", $result,0,"oid") . ").
...
$hours = $today['hours'];
$minutes = $today['minutes'];
SQLact("query", "INSERT INTO freelancers_transactions (amount, details, username, type, balance, date, date2) VALUES ('+" . SQLact("result", $result,0,"amount") . "', '2Checkout Deposit (" . SQLact("result", $result,0,"oid") . ")', '" . SQLact("result", $result,0,"username") . "', 'buyer', '$newbal', '$month/$day/$year  $hours:$minutes', '" . time() . "')");
if ($depositnotify == "enabled") {
mail($emailaddress,$companyname . " 2Checkout Deposit (" . SQLact("result", $result,0,"oid") . ")","The " . SQLact("result", $result,0,"atype") . "\"" . SQLact("result", $result,0,"username") . "\" has just deposited " . $currencytype . "" . $currency . "" . SQLact("result", $result,0,"amount") . " into their account (Order Number: " . SQLact("result", $result,0,"oid") . ").","From: $emailaddress");
}
$today = getdate();
...
echo '<title>Order Error</title><br>
Your 2Checkout transaction could NOT be processed, authorization returned no results for the specified authentication code.';
}
}
?>