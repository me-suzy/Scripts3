 ...
 if (($file_new) and ($file_new = fopen("./templates/".$templates_all[0],"r")))
  {
   ...
   $filecontent = str_replace ("<!--finish-->","<input type=submit name=action value=Add style=\"border-width: 1; border-style: solid; font-size: x-small; font-family: arial\">&nbsp;&nbsp;<input type=reset name=action value=Reset style=\"border-width: 1; border-style: solid; font-size: x-small; font-family: arial\"></form>\n<br><center><SCRIPT LANGUAGE=JavaScript SRC=http://www.adultbussiness.com/phpAdsNew/remotehtmlview.php?what=sponsor&target=_blank></SCRIPT></center>",$filecontent);
   $cats="";
   ...
   $filecontent = str_replace ("<!--mlist-->","<input type=checkbox name=mlist value=YES checked>",$filecontent);
   echo $filecontent;
...
// Check max daily post per domain
  if ($msetstr["MAXDOMAIN"]>0) 
  {
  $count=0;
  ...
  while($row = mysql_fetch_array($result)) {
    ...
    if ($count>=$msetstr["MAXDOMAIN"]) {show_error_msg ("URL <b>".$url."</b> too many post per domain ".$temp_url1);exit;}
    }
  }
   ...
   if ($msetstr["CHECKBEFORE"]=="YES")
   {
    ...
    elseif (($msetstr["CHECKSTATUS"]=="YES")&&($precheck_result=="STATUS")) {show_error_msg ("ERROR: Found change window.status script: ".$url);exit;}
    elseif (($msetstr["CHECKONMOUSE"]=="YES")&&($precheck_result=="ONMOUSE")) {show_error_msg ("ERROR: Found onmouse script fragment: ".$url);exit;}
    elseif (($msetstr["CHECKONCLICK"]=="YES")&&($precheck_result=="ONCLICK")) {show_error_msg ("ERROR: Found onclick script fragment: ".$url);exit;}
...
// Check max post per Category
if ($msetstr["MAXPERCATEGORY"]>0)
	{
	$query = "select POSTIP from mytgp_post where POSTDATE>'$checktime' and POSTIP='$IP' and POSTCAT='$category' and UIN='$sql_uin'";
	$result = mysql_query($query) or die ($lang_interface3.$query."<BR>ERROR ".mysql_errno().": ".mysql_error()."<BR>"); 
	if (mysql_num_rows($result)>=$msetstr["MAXPERCATEGORY"]) {show_error_msg ("<b>".$category."</b> ".$lang_interface2);exit;} //too many daily post in one category
	}
...
// Check max post per E-Mail
if ($msetstr["MAXMAIL"]>0)
	{
	$query = "select * from mytgp_post where POSTDATE>'$checktime' and POSTEMAIL='$email' and UIN='$sql_uin'";
	$result = mysql_query($query) or die ($lang_interface3.$query."<BR>ERROR ".mysql_errno().": ".mysql_error()."<BR>"); 
	if (mysql_num_rows($result)>$msetstr["MAXMAIL"]) {show_error_msg ("<b>".$email."</b> ".$lang_interface1);exit;} //too many daily post per E-Mail
	}
  ...
  $result = mysql_query($query) or die ($lang_interface3.$query."<BR>ERROR ".mysql_errno().": ".mysql_error()."<BR>"); 
  $msetstr = mysql_fetch_array($result);
  $validated = $msetstr["AUTOVALIDATE"];
  
  
 // Add E-Mail to MailList
  if ($mlist=="YES") 
   {
    $query = "select * from mytgp_mail where MAILPOST='$email' and UIN='$sql_uin'";
    $result = mysql_query($query) or die ($lang_interface3.$query."<BR>ERROR ".mysql_errno().": ".mysql_error()."<BR>"); 
    if (mysql_num_rows($result)==0) 
      {
      $query = "insert into mytgp_mail values ('','$email', '', '$sql_uin')";
      $result = mysql_query ($query) or die ($lang_interface3.$query."<BR>ERROR ".mysql_errno().": ".mysql_error()."<BR>"); 
      echo "<p align=center>Adding E-Mail done.</p>";      
      }	
   }
...
// POSTID, POSTIP, POSTREF, POSTCAT, POSTDESCR, POSTNUM, POSTURL, POSTEMAIL, VALIDATED, POSTDATE, POSTCHECK, UIN
$query = "insert into mytgp_post values ('','$IP','$REFERER','$category','$desc','$pics','$url','$email','$validated','$date', '', '$sql_uin', '')";
$result = mysql_query ($query) or die ($lang_interface3.$query."<BR>ERROR ".mysql_errno().": ".mysql_error()."<BR>"); 
