  ...
  class secpay {
    ...
    function process_button() {
      ...
      $process_button_string = xtc_draw_hidden_field('merchant', MODULE_PAYMENT_SECPAY_MERCHANT_ID) .
                               ...
                               xtc_draw_hidden_field('bill_city', $order->billing['city']) .
                               xtc_draw_hidden_field('bill_state', $order->billing['state']) .
                               xtc_draw_hidden_field('bill_post_code', $order->billing['postcode']) .
                               xtc_draw_hidden_field('bill_country', $order->billing['country']['title']) .
                               xtc_draw_hidden_field('bill_tel', $order->customer['telephone']) .
                               xtc_draw_hidden_field('bill_email', $order->customer['email_address']) .
                               xtc_draw_hidden_field('ship_name', $order->delivery['firstname'] . ' ' . $order->delivery['lastname']) .
                               xtc_draw_hidden_field('ship_addr_1', $order->delivery['street_address']) .
                               xtc_draw_hidden_field('ship_addr_2', $order->delivery['suburb']) .
                               xtc_draw_hidden_field('ship_city', $order->delivery['city']) .
                               xtc_draw_hidden_field('ship_state', $order->delivery['state']) .
                               xtc_draw_hidden_field('ship_post_code', $order->delivery['postcode']) .
                               xtc_draw_hidden_field('ship_country', $order->delivery['country']['title']) .
                               xtc_draw_hidden_field('currency', $sec_currency) .
                               xtc_draw_hidden_field('callback', xtc_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL', false) . ';' . xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code, 'SSL', false)) .
                               xtc_draw_hidden_field(xtc_session_name(), xtc_session_id()) .
                               xtc_draw_hidden_field('options', 'test_status=' . $test_status . ',dups=false,cb_post=true,cb_flds=' . xtc_session_name());
