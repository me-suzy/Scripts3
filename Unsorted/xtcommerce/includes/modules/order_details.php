<?php
/* -----------------------------------------------------------------------------------------
   $Id: order_details.php,v 1.1 2003/09/06 22:13:54 fanta2k Exp $   
...
<?php
  ...
  for ($i=0, $n=sizeof($products); $i<$n; $i++) {
    ...
    // Model
    if ((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td valign="top" class="main"><a href="' . xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '">' . $products[$i]['model'] . '</a></td>' . "\n";
    }
  
    // Product name, with or without link
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td valign="top" class="main"><a href="' . xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
    } else {
    ...
    // elari End CS V3.x
    if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
      echo '    <td align="right" valign="top" class="main"><b>' . xtc_get_products_price($products[$i]['id'], $price_special=1, $quantity=$products[$i]['quantity']) . '</b>' . "\n";
    } else {
      echo '    <td align="right" valign="top" class="main"><b>' . xtc_get_products_price($products[$i]['id'], $price_special=1, $quantity=$products[$i]['quantity']) . '</b>' . "\n";
    }
