  ...
  class shoppingCart {
    ...
    function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
      ...
      if ($this->in_cart($products_id)) {
        ...
        if (isset($_SESSION['customer_id'])) xtc_db_query("insert into " . TABLE_CUSTOMERS_BASKET . " (customers_id, products_id, customers_basket_quantity, customers_basket_date_added) values ('" . $_SESSION['customer_id'] . "', '" . $products_id . "', '" . $qty . "', '" . date('Ymd') . "')");
        ...
        if (is_array($attributes)) {
          ...
          while (list($option, $value) = each($attributes)) {
            ...
            // insert into database
            if (isset($_SESSION['customer_id'])) xtc_db_query("insert into " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " (customers_id, products_id, products_options_id, products_options_value_id) values ('" . $_SESSION['customer_id'] . "', '" . $products_id . "', '" . $option . "', '" . $value . "')");
          }
        }
      }
      $this->cleanup();
    ...
    function update_quantity($products_id, $quantity = '', $attributes = '') {
      ...
      // update database
      if (isset($_SESSION['customer_id'])) xtc_db_query("update " . TABLE_CUSTOMERS_BASKET . " set customers_basket_quantity = '" . $quantity . "' where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "'");
      ...
      if (is_array($attributes)) {
        ...
        while (list($option, $value) = each($attributes)) {
          $this->contents[$products_id]['attributes'][$option] = $value;
          // update database
          if (isset($_SESSION['customer_id'])) xtc_db_query("update " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " set products_options_value_id = '" . $value . "' where customers_id = '" . $_SESSION['customer_id'] . "' and products_id = '" . $products_id . "' and products_options_id = '" . $option . "'");
        }
      }
    }
    ...
    function calculate() {
      ...
      while (list($products_id, ) = each($this->contents)) {
        ...
        if ($product = xtc_db_fetch_array($product_query)) {
		  $products_price=xtc_get_products_price($product['products_id'],$price_special=0,$quantity=$qty);
    ...
    function get_products() {
      ...
      reset($this->contents);
      while (list($products_id, ) = each($this->contents)) {
        $products_query = xtc_db_query("select p.products_id, pd.products_name, p.products_model, p.products_price, p.products_discount_allowed, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id='" . xtc_get_prid($products_id) . "' and pd.products_id = p.products_id and pd.language_id = '" . $_SESSION['languages_id'] . "'");
        if ($products = xtc_db_fetch_array($products_query)) {
          $prid = $products['products_id'];
          $products_price = xtc_get_products_price($products['products_id'], $price_special=0, $quantity=1);
