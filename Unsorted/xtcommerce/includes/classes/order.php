<?php
/* -----------------------------------------------------------------------------------------
   $Id: order.php,v 1.3 2003/09/07 10:48:23 fanta2k Exp $   
  ...
  class order {
    ...
    function query($order_id) {
      ...
      while ($orders_products = xtc_db_fetch_array($orders_products_query)) {
        $this->products[$index] = array('qty' => $orders_products['products_quantity'],
					...
					'price'=>xtc_get_products_price($orders_products['products_id'],$price_special=0,$quantity=1),
    ...
    function cart() {
      ...
      $customer_address_query = xtc_db_query("select c.customers_firstname, c.customers_gender,c.customers_lastname, c.customers_telephone, c.customers_email_address, ab.entry_company, ab.entry_street_address, ab.entry_suburb, ab.entry_postcode, ab.entry_city, ab.entry_zone_id, z.zone_name, co.countries_id, co.countries_name, co.countries_iso_code_2, co.countries_iso_code_3, co.address_format_id, ab.entry_state from " . TABLE_CUSTOMERS . " c, " . TABLE_ADDRESS_BOOK . " ab left join " . TABLE_ZONES . " z on (ab.entry_zone_id = z.zone_id) left join " . TABLE_COUNTRIES . " co on (ab.entry_country_id = co.countries_id) where c.customers_id = '" . $_SESSION['customer_id'] . "' and ab.customers_id = '" . $_SESSION['customer_id'] . "' and c.customers_default_address_id = ab.address_book_id");
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                        ...
                                        'price' =>  (xtc_get_products_price($products[$i]['id'],$price_special=0,$quantity=$products[$i]['quantity'])+ xtc_get_products_attribute_price_checkout($_SESSION['cart']->attributes_price($products[$i]['id']),0,0,1,'')*$products[$i]['quantity'])/$products[$i]['quantity'],
                            		'final_price' => xtc_get_products_price($products[$i]['id'],$price_special=0,$quantity=$products[$i]['quantity']) + xtc_get_products_attribute_price_checkout($_SESSION['cart']->attributes_price($products[$i]['id']),0,0,1,'')*$products[$i]['quantity'],
