<?php 
/* --------------------------------------------------------------
   $Id: xsell_products.php,v 1.1 2003/09/06 22:05:29 fanta2k Exp $   
   ...
   (c) 2003	 nextcommerce (xsell_products.php,v 1.6 2003/08/18); www.nextcommerce.org
...
echo '<td class="dataTableContent" valign="top">&nbsp;<a href="' . xtc_href_link(FILENAME_XSELL_PRODUCTS, 'add_related_product_ID=' . $PRODUCTS_id[$i], 'NONSSL') . '">Add</a>&nbsp;</td><td class="dataTableContent" valign="top">&nbsp;<a href="' . xtc_href_link(FILENAME_XSELL_PRODUCTS, 'add_related_product_ID=' . $PRODUCTS_id[$i], 'NONSSL') . '">Remove</a>&nbsp;</td>'; 

if (count($Related_items)>1) 
{ 
echo '<td class="dataTableContent" valign="top">&nbsp;<a href="' . xtc_href_link(FILENAME_XSELL_PRODUCTS, 'sort=1&add_related_product_ID=' . $PRODUCTS_id[$i], 'NONSSL') . '">Sort</a>&nbsp;</td>'; 
} else { 
...
echo '<a href="' . xtc_href_link(FILENAME_XSELL_PRODUCTS, '', 'NONSSL') . '">Click Here to add a new cross sale</a><br>' . "\n"; 
if ($_POST[xsell_id]) 
echo '<a href="' . xtc_href_link(FILENAME_XSELL_PRODUCTS, 'sort=1&add_related_product_ID=' . $_POST[add_related_product_ID], 'NONSSL') . '">Click here to sort (top to bottom) the added cross sale</a>' . "\n"; 
} 

if ($add_related_product_ID && ! $_POST && !$sort) 
{ ?> 
<table border="0" cellpadding="2" cellspacing="1" bgcolor="#999999"> 
<form action="<?php xtc_href_link(FILENAME_XSELL_PRODUCTS, '', 'NONSSL'); ?>" method="post"> 
<tr class="dataTableHeadingRow"> 
<?php 
...
."<td class=\"dataTableContent\">".$currencies->display_price($PRODUCTS_price[$i], xtc_get_tax_rate($product_info_values['products_tax_class_id']))."</td></tr>\n"; 
}?> 
<tr> 
<td colspan="4"> 
...
if ($run_once==0) 
{ 
echo '<b>The Database was updated <a href="' . xtc_href_link(FILENAME_XSELL_PRODUCTS, '', 'NONSSL') . '">Click here to back to the main page</a></b><br>' . "\n"; 
$run_once++; 
} 

}// end of foreach. 
?> 
<form method="post" action="<?php xtc_href_link(FILENAME_XSELL_PRODUCTS, 'sort=1&add_related_product_ID=' . $add_related_product_ID, 'NONSSL'); ?>"> 
...
<td class="dataTableContent"><?php echo $currencies->display_price($PRODUCTS_price[0], xtc_get_tax_rate($product_info_values['products_tax_class_id'])); ?></td> 
