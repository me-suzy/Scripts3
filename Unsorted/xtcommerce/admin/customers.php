<?php
/* --------------------------------------------------------------
   $Id: customers.php,v 1.1 2003/09/06 22:05:29 fanta2k Exp $   
  ...
  if ($_GET['special'] == 'remove_memo') {
    ...
    xtc_redirect(xtc_href_link(FILENAME_CUSTOMERS, 'cID=' . $_GET['cID'] . '&action=edit'));
  }
  ...
  if ($_GET['action']) {
    ...
    //Temporarily set due to above commented lines
      ...
      default:
        $customers_query = xtc_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . $_GET['cID'] . "'");
...
<?php
  if ($_GET['action'] == 'edit' || $_GET['action'] == 'update') {
    $customers_query = xtc_db_query("select c.customers_gender,c.customers_status, c.member_flag, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . $_GET['cID'] . "'");
...
<?php include(DIR_WS_MODULES . FILENAME_CUSTOMER_MEMO); ?>
      ...
      <tr>
        <td align="right" class="main"><?php echo xtc_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('action'))) .'">' . xtc_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
      </tr></form>
<?php
  } else {
?>
    ...
    <td class="main" valign="top">XT Customers</td>
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr><?php echo xtc_draw_form('search', FILENAME_CUSTOMERS, '', 'get'); ?>
            <td class="pageHeading"><?php echo '<a href="' . xtc_href_link(FILENAME_CREATE_ACCOUNT) . '">' . xtc_image_button('create_account.gif', CREATE_ACCOUNT) . '</a>'; ?></td>
...
?>          
            <td class="smallText" align="right"><?php echo HEADING_TITLE_STATUS . ' ' . xtc_draw_pull_down_menu('status',xtc_array_merge($select_data, $customers_statuses_array), '99', 'onChange="this.form.submit();"'); ?></td>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_GROUPIMAGE; ?></td>
...
<?php
    ...
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, a.entry_country_id, c.customers_status, c.member_flag from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . $search . " order by c.customers_lastname, c.customers_firstname";
...
?>
                <td class="dataTableContent"><?php if ($customers_statuses_array[$customers['customers_status']]['csa_image'] != '') { echo xtc_image(DIR_WS_ICONS . $customers_statuses_array[$customers['customers_status']]['csa_image'], ''); } ?>&nbsp;</td>
...
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a href="' . xtc_href_link(FILENAME_CUSTOMERS) . '">' . xtc_image_button('button_reset.gif', IMAGE_RESET) . '</a>'; ?></td>
                  </tr>
<?php
    }
?>
...
//      echo 'customer_status ' . $cID . 'variables = ' . $cs_id . $cs_member_flag . $cs_name .  $cs_discount .  $cs_image . $cs_ot_discount;
      ...
      if (is_object($cInfo)) {
        ...
        if ($cInfo->customers_id != 1) {
          $contents[] = array('align' => 'center', 'text' => '<a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . xtc_image_button('button_edit.gif', IMAGE_EDIT) . '</a>');
        }
        if ($cInfo->customers_id == 1 && $_SESSION['customer_id'] == 1) {
          $contents[] = array('align' => 'center', 'text' => '<a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . xtc_image_button('button_edit.gif', IMAGE_EDIT) . '</a>');
        }
        if ($cs_id != 0) {
          $contents[] = array('align' => 'center', 'text' => '<a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' . xtc_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
        }
        if ($cInfo->customers_id != 1 && $_SESSION['customer_id'] == 1) {
          $contents[] = array('align' => 'center', 'text' => '<a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=editstatus') . '">' . xtc_image_button('button_status.gif', IMAGE_STATUS) . '</a>');
        }
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . xtc_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' . xtc_image_button('button_orders.gif', IMAGE_ORDERS) . '</a> <a href="' . xtc_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . xtc_image_button('button_email.gif', IMAGE_EMAIL) . '</a><br><a href="' . xtc_href_link(FILENAME_CUSTOMERS, xtc_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=iplog') . '">' . xtc_image_button('button_iplog.gif', IMAGE_IPLOG) . '</a>');
