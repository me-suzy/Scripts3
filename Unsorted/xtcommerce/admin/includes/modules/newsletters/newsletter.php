...
//--></script>';
      ...
      $choose_audience_string .= '<form name="notifications" action="' . xtc_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID'] . '&action=confirm') . '" method="post" onSubmit="return selectAll(\'notifications\', \'chosen[]\')"><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n" .
                                 '  <tr>' . "\n" .
                                 '    <td align="center" class="main"><b>' . TEXT_CUSTOMERS_STATUS . '</b><br>' . xtc_draw_pull_down_menu('cstatuses', $customers_statuses_array, '', 'size="20" style="width: 20em;" multiple') . '</td>' . "\n" .
                                 ...
                                 '    <td align="center" class="main"><b>' . TEXT_SELECTED_CUSTOMERS_STATUS . '</b><br>' . xtc_draw_pull_down_menu('chosen[]', array(), '', 'size="20" style="width: 20em;" multiple') . '</td>' . "\n" .
                                 '  </tr>' . "\n" .
                                 '</table></form>';
    ...
    function confirm() {
      ...
      $mail_query = xtc_db_query("select count(*) as count from " . TABLE_CUSTOMERS . " where customers_newsletter = '1'");
      ...
      $confirm_string = '<table border="0" cellspacing="0" cellpadding="2">' . "\n" .
                        ...
                        '  </tr>' . "\n" .
                        '  <tr>' . "\n" .
                        '    <td align="right"><a href="' . xtc_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID'] . '&action=confirm_send') . '">' . xtc_image_button('button_send.gif', IMAGE_SEND) . '</a> <a href="' . xtc_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . xtc_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a></td>' . "\n" .
                        '  </tr>' . "\n" .
                        '</table>';
