...
function countref($tname,$cfdomain,$cdomain,$cuniquetime,$cunique_strict) {
  Global $thour,$min,$setUcookie,$setRcookie;
...
function cookieUnique($time) {
	?>
	document.cookie = "unique_strict=true; expires=<?php echo date("l, j-M-y G:i:s ", time()+$time); ?>GMT;"
	<?
}
function cookieRefer($user) {
	?>
	document.cookie = "refer_strict=<?=$user?>; expires=<?php echo date("l, j-M-y G:i:s ", time()+86400); ?>GMT;"
	<?
}
?>
//-->
