...
// ---------------------------------------------------- IF MODIFY STAFF PASSWORD -------------------------------------------------------------------

if ($password){

$encryptedpassword = md5($password);

$sql = "UPDATE staff SET password='$encryptedpassword' WHERE id=$id";
$result = mysql_query($sql);
...
echo "<script>window.setTimeout('changeurl();',2000); function changeurl(){window.location='viewstaff.php?id=$id'}</script>";

// ----------------------------------------------------- END IF MODIFY STAFF PASSWORD -------------------------------------------------------------

// ------------------------------------------------------- IF MODIFY STAFF ------------------------------------------------------------------------

} elseif ($submit) {

$sql = "UPDATE staff SET adminname='$adminname', access='$new_access' WHERE id=$id";
