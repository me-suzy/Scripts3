...
#///////////////////////////////////////////////////////
	...
	if(!$id) { $id = $sessionVars["SELL_auction_id"]; }
...
$query = "select * from PHPAUCTIONPROPLUS_auctions where id='$id'";
	...
	if(mysql_result ( $result, 0, "reserve_price" ) > 0) 
		{ $with_reserve = "yes";
		} else
		{ $with_reserve = "no";
		}

	if(mysql_result ( $result, 0, "international" ) == 0) 
		{ $with_reserve = "no";
		} else
		{ $with_reserve = "yes";
		}
	$auc_id = $id;
	$sessionVars["SELL_auction_id"] = $id;
	$sessionVars["SELL_suspended"] = mysql_result ( $result, 0, "suspended" );
	$sessionVars["SELL_title"] = mysql_result ( $result, 0, "title" );
	$sessionVars["SELL_description"] = mysql_result ( $result, 0, "description" );
	$sessionVars["SELL_atype"] = mysql_result ( $result, 0, "auction_type" );
	$sessionVars["SELL_iquantity"] = mysql_result ( $result, 0, "quantity" );
	$sessionVars["SELL_minimum_bid"] = eregi_replace('.0000',"", mysql_result ( $result, 0, "minimum_bid" ));
	$sessionVars["SELL_iquantity"] = mysql_result ( $result, 0, "quantity" );
	$sessionVars["SELL_suspended"] = mysql_result ( $result, 0, "suspended" );
	$sessionVars["SELL_category"] = mysql_result ( $result, 0, "category" );
	$sessionVars["SELL_reserve_price"] = eregi_replace('.0000',"", mysql_result ( $result, 0, "reserve_price" ));
	$sessionVars["SELL_with_reserve"] = $with_reserve;
	$sessionVars["SELL_location_zip"] = mysql_result ( $result, 0, "location_zip" );
	$sessionVars["SELL_shipping"] = mysql_result ( $result, 0, "shipping" );
	$sessionVars["SELL_international"] = $international;
	$sessionVars["SELL_customincrement"] = eregi_replace('.0000',"", mysql_result ( $result, 0, "increment" ));
...
#			}
	...
	*/
	if ( empty($action) || (($action=='first')&&($$ERR)) )
	{
		...
		include "templates/template_bulkuploadauction_php.html";
		include "footer.php";
		exit;
	}
...
*/
		...
		include "header.php";
		include "templates/template_bulkuploadauction_preview_php.html";
		include "footer.php";
		exit;
	}
	...
	if ($action=='second' && !$$ERR)
	{
		...
		if ( !$sessionVars["SELL_DATA_CORRECT"] )
			header ( "Location: bulkuploadauction.php" );
		...
		else
		{
			...
			if($SETTINGS[sellersetupfee] == 1)
			{
				...
				{
					include "templates/template_bulkuploadauction_result_pay_php.html";
				}
			}
			else
			{
				include "templates/template_bulkuploadauction_result_php.html";
			}
		}
