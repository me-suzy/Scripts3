...
/* get auction data  */
   $query = "select * from PHPAUCTIONPROPLUS_auctions where id=\"$auction_id\"";
   $result = mysql_query($query);
   if ( !$result )
   {
		MySQLError($query);
...
/* get bids for current item */
   $query       = "select max(bid) as maxbid from PHPAUCTIONPROPLUS_bids where auction=\"$auction_id\" group by auction";
   ...
   {
		/* Get number of bids  */
		$query          = "select * from PHPAUCTIONPROPLUS_bids where auction=\"$auction_id\"ORDER BY id DESC";
...
//-- Display bids history               
               	...
               	while  ( $i < $num_bids )
				{
                    ...
                    $query = "select id,nick from PHPAUCTIONPROPLUS_users where id=\"$bidder\"";
                    $result_bidder = mysql_query($query);
                    if(!$result)
					{
						MySQLError($query);
