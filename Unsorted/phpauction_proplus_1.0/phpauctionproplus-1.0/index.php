...
#///////////////////////////////////////////////////////
	...
	*/
	...
	if($user_email){
		if(!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+([\.][a-z0-9-]+)+$",$user_email)){
			...
			$query = "insert into PHPAUCTIONPROPLUS_info values(\"$user_email\")";
	...
	*/
	...
	/* prepare categories list for templates/template */
		...
		$query = "select * from PHPAUCTIONPROPLUS_categories WHERE parent_id=0 order by sub_counter desc";
		$result = mysql_query($query);
			if(!$result)
			{
				MySQLError($query);
...
#					print $sub_count."<BR>";
	...
	/* get last created auctions */
			$query = "select id,title,starts from PHPAUCTIONPROPLUS_auctions where closed='0' and suspended=0 order by starts desc";
			$result = mysql_query($query);
			if ( $result )
	...
	/* get ending soon auctions */
			...
			$query = "select ends,id,title from PHPAUCTIONPROPLUS_auctions where closed=\"0\" and
	...
	/* get higher bids */
			...
			$TPL_maximum_bids = "";
			$query = "select auction,max(bid) as max_bid from PHPAUCTIONPROPLUS_bids group by auction order by max_bid desc";
			$result = mysql_query($query);
			...
			while($i < $num_auction && $j < 6){
				...
				$query = "select title,closed,id from PHPAUCTIONPROPLUS_auctions where id=\"$auction\"";
	...
	{
		$query = "SELECT title,id from PHPAUCTIONPROPLUS_news where suspended=0 order by new_date limit $SETTINGS[newstoshow]";
