...
#///////////////////////////////////////////////////////
	...
	$query = "select bid from PHPAUCTIONPROPLUS_bids where auction=\"$id\"";
	$result = mysql_query($query);
...
/*	} */
    ...
    if(file_exists($uploaded_path.$id))
    {
      ...
      $GALLERY_DIR = $id;
      session_name($SESSION_NAME);
      session_register(UPLOADED_PICTURES,GALLERY_DIR);
       
       if(count($UPLOADED_PICTURES) > 0)
       {
           $TPL_gallery = "<A HREF=\"Javascript:window_open('gallery.php?id=$id','gallery',650,150,10,01)\"><IMG SRC=images/gallery.png BORDER=0></A>&nbsp;<A HREF=\"Javascript:window_open('gallery.php?id=$id','gallery',650,150,10,01)\">$MSG_694</A>";
       }
       else
       {
           $TPL_gallery = "";
	...
	if ($buy_now_price > 0 && !isset($TPL_BIDS_value))
	{
		$buy_now_price = print_money($buy_now_price);
		$TPL_buy_now_price = "$buy_now_price  <a href=\"buy_now.php?id=$id\"><img border=\"0\" align=\"absbottom\" alt=\"$MSG_496\" src=\"./images/buy_it_now.gif\"></a>";
	}
	else 
	{
...
*/
	...
	if ( mysql_num_rows($result_bids ) > 0)
	{
	   ...
	   $query          = "select bidder from PHPAUCTIONPROPLUS_bids where bid=$high_bid and auction='$id'";
	...
	else
	{
	   ...
	   /* Get bidder nickname */
	   $query         = "select id,nick from PHPAUCTIONPROPLUS_users where id='$high_bidder_id'";
	   $result_bidder = mysql_query($query);
	   if ( !$result_bidder )
	   {
			MySQLError($query);
   ...
   if ( $pict_url )
	  ...
	  if ( $high_bidder_id ){
	   ...
	   /* Get current number of feedbacks */
	   $query		   = "select rated_user_id from PHPAUCTIONPROPLUS_feedbacks where rated_user_id=\"$high_bidder_id\"";
	   $result		   = mysql_query ( $query );
	   $num_feedbacks = mysql_num_rows($result);

	   /* Get current total rate value for user */
	   $query = "select rate_sum,nick,id from PHPAUCTIONPROPLUS_users where nick=\"$high_bidder_nick\"";
	   $result = mysql_query($query);
	...
	$query = "SELECT cat_id,parent_id,cat_name FROM PHPAUCTIONPROPLUS_categories WHERE cat_id='$category'";
	$result = mysql_query($query);
	if ( !$result )
	{
		MySQLError($query);
	...
	$i = 0;
	do {
				$query = "SELECT cat_id,parent_id,cat_name FROM PHPAUCTIONPROPLUS_categories WHERE cat_id='$j'";
			 $result = mysql_query($query);
			 if ( !$result )
			{
				MySQLError($query);
