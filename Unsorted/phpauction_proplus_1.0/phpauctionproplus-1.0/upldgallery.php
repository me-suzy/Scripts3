...
#///////////////////////////////////////////////////////
    ...
    {
        unlink($image_upload_path.session_id()."/".$UPLOADED_PICTURES[$img]);
        
        unset($UPLOADED_PICTURES[$img]);
        unset($UPLOADED_PICTURES_SIZE[$img]);
        session_name($SESSION_NAME);
        
        session_register(UPLOADED_PICTURES,UPLOADED_PICTURES_SIZE);
    }
    ...
    if(!empty($HTTP_POST_VARS[creategallery]))
    {
        ...
        session_name($SESSION_NAME);
        session_register(GALLERY_UPDATED);
...
<TR>
  <TD COLSPAN=2>
      <?
        print $sml_font.$MSG_673."&nbsp;".$SETTINGS[maxpictures]."&nbsp;".$MSG_674; 
        if($SETTINGS[picturesgalleryfee] == 1 )
        {
            print $MSG_675."&nbsp;".print_money($SETTINGS[picturesgalleryfee])."&nbsp;".$MSG_676;
        }
        print "<BR>".$MSG_679;
      ?>
  </TD>
...
<?
  if(count($UPLOADED_PICTURES) < $SETTINGS[maxpictures])
  {
?>
    <TR>
      <TD>
...
?>  
  <TR>
    <TD ALIGN=CENTER>
        <?=$err_font?><?=$MSG_688."&nbsp;".$SETTINGS[maxpictures]."&nbsp;".$MSG_689?></FONT>
    </TD>
  </TR>
<?
  }
