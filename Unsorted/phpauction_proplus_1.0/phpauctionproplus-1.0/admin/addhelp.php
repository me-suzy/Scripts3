...
#///////////////////////////////////////////////////////
   ...
   require('../includes/config.inc.php');
	...
	if ($act == "save") {
	   $sqlstr = "INSERT INTO PHPAUCTIONPROPLUS_help (topic,helptext) VALUES ('";
	...
	if ($act == "update") {
	   $sqlstr = "UPDATE PHPAUCTIONPROPLUS_help set helptext = '";
...
<?
	if ($TPL_error) {
        ...
        if ($act == "edit") {
		$query = "select topic,helptext from PHPAUCTIONPROPLUS_help where topic='" . $topic . "';";
		$result = mysql_query($query);
		if(!$result){
			print "$ERR_001<BR>$query<BR>".mysql_error();
	 		include "./footer.php";
