...
require("header.php");
	...
	if(!$action || $$ERR){
		...
		$query = "select * from PHPAUCTIONPROPLUS_users where nick=\"$user\"";
   	...
   	$query = "select * from PHPAUCTIONPROPLUS_countries order by name";
   	$result = mysql_query($query);
   	if(!$result)
   	{
			MySQLError($query);
	...
	if($action && !$$ERR){
		...
		$query = "update PHPAUCTIONPROPLUS_users set name=\"$name\", 
										   address=\"$address\", 
										   city=\"$city\", 
										   prov=\"$prov\", 
										   country=\"$country\", 
   	...
   	$query = "select name from PHPAUCTIONPROPLUS_countries where code>=\"$country\"";
   	$result = mysql_query($query);
