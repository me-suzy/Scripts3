...
*/
	...
	if ($id==0)
	{
		...
		*/
		$result = mysql_query ( "SELECT * FROM PHPAUCTIONPROPLUS_categories WHERE parent_id='0' ORDER BY cat_name" );
		if (!$result)
		{
	...
	else
	{
		...
		*/
		$result = mysql_query ( "SELECT * FROM PHPAUCTIONPROPLUS_categories WHERE cat_id='$id'" );
		if ($result)
		...
		else
		{
			...
			*/
			...
			{
				// get next parent
				$res = mysql_query ( "SELECT * FROM PHPAUCTIONPROPLUS_categories WHERE cat_id='$par_id'");
				if ($res)
				{
			...
			/* get list of subcategories of this category */
					$subcat_count = 0;
					$result = mysql_query ( "SELECT * FROM PHPAUCTIONPROPLUS_categories WHERE parent_id='$id' ORDER BY cat_name" );
					if (!$result)
					{
						/* output error message and exit */
					}
			...
			/* get list of auctions of this category */
					$auctions_count = 0;
						...
						if ($lines==0)	$lines = 50;
							...
							$rsl = mysql_query ( "SELECT count(*) FROM PHPAUCTIONPROPLUS_auctions WHERE category='$id' AND closed='0'" );
							if ($rsl)
							{
					...
					$result = mysql_query ( "SELECT * FROM PHPAUCTIONPROPLUS_auctions WHERE category='$id' AND closed='0' ORDER BY starts DESC LIMIT $left_limit,$lines" );
