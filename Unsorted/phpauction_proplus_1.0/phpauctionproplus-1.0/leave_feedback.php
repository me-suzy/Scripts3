...
$query = "select user_id from PHPAUCTIONPROPLUS_feedbacks where user_id=\"$user\"";
...
$query = "select rate from PHPAUCTIONPROPLUS_users where nick=\"$user\"";
$result = mysql_query($query);
...
if($action && !$$ERR){
	...
	$query = "insert into PHPAUCTIONPROPLUS_feedbacks values (\"$user\", \"$other_user\", \"$feedback_text\", \"$rate\", \"$today\")";

	$result = mysql_query($query);
	
	$total_rate = $total_rate + intval($rate);
	//--
	$query = "update PHPAUCTIONPROPLUS_users set rate=$total_rate, where nick=\"$user\"";
