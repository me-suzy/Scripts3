...
<TABLE>
<TR>
<TD align=center>Enter up to 10 DMOZ directories below:</TD>
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="70%" id="AutoNumber1">
  <tr>
    <td width="100%"><font size="2" face="Arial">Please enter one or more Dmoz directories to see 
...
Valid Dmoz directory: <br>
http://dmoz.org/Arts/Architecture/History/</font><p><font face="Arial" size="2">To download the entire list of Yahoo directory, 
right click the file below and select &quot;<b>Save target as</b>&quot;<br>
<a href="dmoz.txt">Dmoz Full Directory File</a> (~197KB) </font></p></td>
  </tr>
</table>
<?
...
foreach ($directories as $directory) {
 ...
 if (!preg_match("/^http\:\/\/dmoz\.(org|com)\//", $directory)) {
  print "Skipping.  Not a valid DMOZ directory.\n";
  continue;
 }
 ...
 foreach ($content as $line) {
  if (preg_match("/<li><a\s+href\=/", $line)) {
   $result = "";
   $line = preg_replace("/^.*\*http:\/\//", "", $line);
   preg_match("/([\w\d-])*\.(com|net|org)(\"|\/)/", $line, $matches);
   $domain = @ $matches[0];
   $domain = preg_replace("/(\"$|\/$)/", "", $domain);
   if ($domain == "dmoz.com" || $domain == "dmoz.org" || $domain == "") {
    continue;
   }
   $counter++;
