...
if(isset($user_user)){
	...
	if (trim($password)==trim($pass_user)){
		...
		//get account title
		$query="SELECT custom,usergroup FROM $user_table WHERE username = '$user_user';";
		$result=mysql_query($query,$link);
		$a_row=mysql_fetch_array($result);
		$account_title=$username;
		$usergroup=$a_row['usergroup'];
		if($usergroup!="3"){
			die("The username that you are logged in under is not an administrator.<!--Pos 5*=>$usergroup-->");
		}
		else{
			$logged_in=true;
		}
