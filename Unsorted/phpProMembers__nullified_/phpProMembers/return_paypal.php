...
$name = $GLOBALS["name"];
$account_id = $GLOBALS["a"];

$temp_data = explode("_", $name);
...
if ($c == "1") {
	...
	$clean_up_sql = "DELETE FROM `members_waiting_approval` WHERE first_name = \"$fname\" AND last_name = \"$lname\"";
...
if ($automatic_approval == "1") {
	$fp = fopen("$temp_directory/temp_".$fname."_".$lname.".txt", "r");
	if (!$fp) {
		mysql_query($clean_up_sql);
	...
	$temp_order_data = explode("|", $order);
	$fname = $temp_order_data[0];
	$lname = $temp_order_data[1];

	$zip = $temp_order_data[2];
