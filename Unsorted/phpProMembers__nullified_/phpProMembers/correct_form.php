...
$check_and_see_if_email_exists_sql = "SELECT * FROM members WHERE e_mail_address = \"$email\"";
$result = mysql_query($check_and_see_if_email_exists_sql);
while($row = mysql_fetch_object($result)) {
	if ($row->e_mail_address == "$email") {
		echo "<center><b>Please choose another e-mail address.  $email is already taken.</b></center><br>";
		exit;
	}
}
...
// Checking to see if user name is already taken.
$check_for_user_sql = "SELECT * from members WHERE user_name = \"$username\"";
$result = mysql_query($check_for_user_sql);
while($row = mysql_fetch_object($result)) {
	if ($row->user_name == "$username") {
		echo "<center><b>Please choose another user name.  $username is already taken.</b></center><br>";
		exit;
	}
}
?>	
...
<TABLE>
	...
	<TR>
    	<TD>
			<FONT FACE="verdana, arial, helvetica">User Name</font> <?php echo $username ?>
		</TD>
    </TR>
	<TR>
    	<TD>
			<FONT FACE="verdana, arial, helvetica">Password:</font> <?php echo $pwd ?>
		</TD>
    </TR>
</TABLE>
