<? //MySQL installation - tables definition
	...
	db_query("CREATE TABLE ".PRODUCTS_TABLE." (productID INT PRIMARY KEY AUTO_INCREMENT, categoryID INT, name VARCHAR(255), description TEXT, customers_rating FLOAT, Price FLOAT, picture VARCHAR(30), in_stock INT, thumbnail VARCHAR(30), customer_votes INT, items_sold INT, show_as_special_offer INT, big_picture VARCHAR(30), enabled INT, brief_description TEXT, list_price FLOAT, product_code CHAR(25))") or die (db_error());
	...
	db_query("CREATE TABLE ".NEWS_TABLE." (NID INT PRIMARY KEY AUTO_INCREMENT, add_date VARCHAR(30), Body TEXT, add_stamp INT)") or die (db_error());
	db_query("CREATE TABLE ".DISCUSSIONS_TABLE." (DID INT PRIMARY KEY AUTO_INCREMENT, productID INT, Author VARCHAR(40), Body TEXT, add_time DATETIME, Topic VARCHAR(255))") or die (db_error());
	db_query("CREATE TABLE ".MAILING_LIST_TABLE." (MID INT PRIMARY KEY AUTO_INCREMENT, Email VARCHAR(50))") or die (db_error());
	...
	db_query("CREATE TABLE ".PRODUCT_OPTIONS_TABLE." (optionID INT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(50))") or die (db_error());
	...
	db_query("CREATE TABLE ".SHIPPING_METHODS_TABLE." (SID INT PRIMARY KEY AUTO_INCREMENT, Name VARCHAR(30), description VARCHAR(255), lump_sum FLOAT, percent_value FLOAT, Enabled INT)") or die (db_error());
	db_query("CREATE TABLE ".PAYMENT_TYPES_TABLE." (PID INT PRIMARY KEY AUTO_INCREMENT, Name VARCHAR(30), description VARCHAR(255), Enabled INT, calculate_tax INT)") or die (db_error());
