...
switch($fn)
{
			...
			if(isset($form_options['submit']))
			{
				if(!$errors = check_input_register())
				{
					...
					if($OPTIONS['USE_EMAIL_ACTIVATION_SYSTEM'])
					{
						...
						$db->insert_from_array($db_stuff, $DB_TABLES['USERS']);
						$user_id = $db->query_one_result("SELECT id FROM $DB_TABLES[USERS] WHERE username='$db_stuff[username]' ORDER BY id DESC LIMIT 1");
	...
	case 'edit_profile':
		...
		if(isset($form_options['submit']))
		{
			if(!$errors = check_input_edit_profile())
			{
				...
				$db->update_from_array($db_stuff, $DB_TABLES['USERS'], $form_options['id']);
		...
		{
			$form_options = load_data_from_db($DB_TABLES['USERS'], $form_options['id']);
	...
	case 'show_profile':
		...
		$result = $db->query("SELECT * FROM $DB_TABLES[USERS] WHERE id='$user_id'");
		if(!$result->num_rows())
		{
		...
		$result = $db->query("SELECT $DB_TABLES[GAMES].id,$DB_TABLES[GAMES].name,$DB_TABLES[GAME_SESSIONS].start_time, $DB_TABLES[GAME_SESSIONS].score
					FROM $DB_TABLES[GAMES], $DB_TABLES[GAME_SESSIONS] 
					WHERE $DB_TABLES[GAMES].id=$DB_TABLES[GAME_SESSIONS].game_id 
					AND $DB_TABLES[GAME_SESSIONS].user_id='$user_id' 
					AND $DB_TABLES[GAME_SESSIONS].end_type=" . TS_GAME_END_TYPE_NORMAL . "
					ORDER BY $DB_TABLES[GAME_SESSIONS].id DESC LIMIT 1");
		if($result->num_rows())
		{
			$last_game_info = $result->fetch_array();
			$last_game_played = new ts_template("LAST_GAME_PLAYED");
			$last_game_played->set("LAST_GAME_LINK", $TS_SCRIPTS['GAME_INFO'] . "?game_id=$last_game_info[id]");
	...
	case 'ac':
		...
		$result = $db->query("SELECT username, level, activation_key FROM $DB_TABLES[USERS] WHERE id=$user_id && activation_key=$key");
		...
		{
			$db->query("UPDATE $DB_TABLES[USERS] SET level=" . TS_USER_LEVEL_NORMAL . " WHERE id=$user_id");
			$username = $db->query_one_result("SELECT username FROM $DB_TABLES[USERS] WHERE id=$user_id");
...
{
	global $form_options,$form_options_error,$OPTIONS, $db, $DB_TABLES;
	if( $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[USERS] WHERE username='$form_options[username]' AND level != " . TS_USER_LEVEL_PENDING) )
	{
	...
	elseif($db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[USERS] WHERE email='$form_options[email]' AND level != " . TS_USER_LEVEL_PENDING))
	{
