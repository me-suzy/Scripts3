...
define("TS_GAME_RESPONSE_TIME_LIMIT", 180);
...
define('TS_VERSION', '1.2.3'); // version of this software
...
$DB_TABLES = array(
	...
	'ANSWERS'		=> 'ts_answers',
	'GAMES'			=> 'ts_games',
	'GAME_CATS'		=> 'ts_game_cats',
	'USERS'			=> 'ts_users',
	'OPTIONS'		=> 'ts_options',
	'OPTION_GROUPS'		=> 'ts_option_groups',
	'TEMPLATE_VARS'		=> 'ts_template_vars',
	'TEMPLATES'		=> 'ts_templates',
	'TEMPLATE_CACHE'	=> 'ts_template_cache',
	'GAME_SESSIONS'		=> 'ts_game_sessions',
	'GAME_SECTIONS'		=> 'ts_game_sections',
	'GSQ'			=> 'ts_gsq',
	'GAME_QUESTION_CACHE'	=> 'ts_game_question_cache',

	'BLACKBOARD'		=> 'ts_blackboard'
	); // end array()
$TS_SCRIPTS = array(

	'INDEX'			=> 'index'			. $script_ext,
	'GAME_INFO'		=> 'game_info'			. $script_ext,
	'USER'			=> 'user'			. $script_ext,
	'TRIVIA'		=> 'trivia'			. $script_ext,
	'PLAY'			=> 'play'			. $script_ext,
...
function get_templates()
{
	...
	$result = $db->query("SELECT the_key,data FROM $DB_TABLES[TEMPLATE_CACHE]");
...
function cache_templates()
{
	...
	global $db, $DB_TABLES;
	$result = $db->query("SELECT the_key,data FROM $DB_TABLES[TEMPLATES]");
	$result2 = $db->query("SELECT the_key,the_value FROM $DB_TABLES[TEMPLATE_VARS] ORDER BY id ASC");
	$result3 = $db->query("SELECT the_key,the_value FROM $DB_TABLES[OPTIONS]");

	$db->query("DELETE FROM $DB_TABLES[TEMPLATE_CACHE]");
	...
	while( $row = $result->fetch_array() )
	{
		...
		$db->query("INSERT INTO $DB_TABLES[TEMPLATE_CACHE] (the_key,data) VALUES ('$row[the_key]', '$row[data]')");
...
function get_options($option="")
{
	...
	if($option)
	{
		return $db->query_one_result("SELECT the_value FROM $DB_TABLES[OPTIONS] WHERE the_key='$option'");
	}

	$result = $db->query("SELECT the_key, the_value FROM $DB_TABLES[OPTIONS]");
...
function get_num_players($game_id=0)
{
	...
	if($game_id)
		return (int)$db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[GAME_SESSIONS] WHERE game_id=$game_id AND end_type=" . TS_GAME_END_TYPE_NOT_ENDED);
	 else
		return (int)$db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[GAME_SESSIONS] WHERE end_type=" . TS_GAME_END_TYPE_NOT_ENDED);
}
function num_games_using_category($category_id)
{
	global $db, $DB_TABLES;

	return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[GAMES],$DB_TABLES[GAME_CATS] WHERE $DB_TABLES[GAMES].id = $DB_TABLES[GAME_CATS].game_id AND $DB_TABLES[GAME_CATS].category_id=$category_id AND $DB_TABLES[GAMES].mode !=" . TS_GAME_MODE_OFFLINE);
...
function get_games_using_category($category_id)
{
	...
	global $db,$DB_TABLES;
	$result = $db->query("SELECT $DB_TABLES[GAMES].name, $DB_TABLES[GAMES].id FROM $DB_TABLES[GAMES],$DB_TABLES[GAME_CATS] WHERE $DB_TABLES[GAMES].id=$DB_TABLES[GAME_CATS].game_id AND $DB_TABLES[GAME_CATS].category_id=$category_id AND $DB_TABLES[GAMES].mode !=" . TS_GAME_MODE_OFFLINE);
...
function game_in_use($game_id)
{
	...
	$mode = $db->query_one_result("SELECT mode FROM $DB_TABLES[GAMES] WHERE id=$game_id");
	...
	switch($mode)
	{
		...
		case TS_GAME_MODE_MAINTENANCE:
			if( $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[GAME_SESSIONS] WHERE game_id=$game_id AND end_type=" . TS_GAME_END_TYPE_NOT_ENDED) )
				return 1;
			else
				return 0;
		break;
...
function get_category_name($category_id)
{
	...
	return $db->query_one_result("SELECT name FROM $DB_TABLES[CATEGORIES] WHERE id=$category_id");
...
function get_game_name($game_id)
{
	...
	return $db->query_one_result("SELECT name FROM $DB_TABLES[GAMES] WHERE id=$game_id");
...
function get_user_id($username)
{
	...
	return $db->query_one_result("SELECT id FROM $DB_TABLES[USERS] WHERE username='$username'");
...
function reset_game_scores($game_id=0)
{
	...
	if($game_id)
	{
		$db->query("UPDATE $DB_TABLES[GAME_SESSIONS] SET game_type=" . TS_GAME_TYPE_OLD . " WHERE game_id=$game_id AND game_type=" . TS_GAME_TYPE_NORMAL);
		$db->query("UPDATE $DB_TABLES[GAMES] SET last_score_reset=$time WHERE id=$game_id");
	}
	else
	{
		$db->query("UPDATE $DB_TABLES[GAME_SESSIONS] SET game_type=" . TS_GAME_TYPE_OLD . " WHERE game_type=" . TS_GAME_TYPE_NORMAL);
		$db->query("UPDATE $DB_TABLES[GAMES] SET last_score_reset=$time WHERE id=$game_id");		
	}
...
{	
	...
	if($game_id)
	{
		$db->query("DELETE FROM $DB_TABLES[GAME_SESSIONS] WHERE game_id=$game_id");
		$db->query("UPDATE $DB_TABLES[GAMES] SET plays=0 WHERE id=$game_id");
	}
	else
	{
		$db->query("DELETE FROM $DB_TABLES[GAME_SESSIONS]");
		$db->query("UPDATE $DB_TABLES[GAMES] SET plays=0");
	}
...
function set_game_mode($mode, $game_id)
{
	...
	if($game_id)
	{
		$db->query("UPDATE $DB_TABLES[GAMES] SET mode=$mode WHERE id=$game_id");
	}
	else
	{
		$db->query("UPDATE $DB_TABLES[GAMES] SET mode=$mode");
	}
...
{	
	...
	if($game_id)
	{
		$result = $db->query("SELECT id FROM $DB_TABLES[GAME_SESSIONS] WHERE game_id=$game_id AND end_type=" . TS_GAME_END_TYPE_NOT_ENDED);
		$db->query("UPDATE $DB_TABLES[GAME_SESSIONS] SET end_type=" . TS_GAME_END_TYPE_INTERRUPTED . ", end_time='" . time() . "' WHERE end_type=" . TS_GAME_END_TYPE_NOT_ENDED . " AND game_id=$game_id");

	}
	else
	{
		$result = $db->query("SELECT id FROM $DB_TABLES[GAME_SESSIONS] WHERE end_type=" . TS_GAME_END_TYPE_NOT_ENDED);
		$db->query("UPDATE $DB_TABLES[GAME_SESSIONS] SET end_type=" . TS_GAME_END_TYPE_INTERRUPTED . ", end_time='" . time() . "' WHERE end_type=" . TS_GAME_END_TYPE_NOT_ENDED);
	}
	while(@list($id) = $result->fetch_array())
	{
		$db->query("DROP TABLE IF EXISTS $DB_TABLES[GSQ]_$id");
	}
...
function game_exists($id)
{
	...
	return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[GAMES] WHERE id=$id");
...
function category_exists($id)
{
	...
	return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[CATEGORIES] WHERE id=$id");
...
function question_exists($id)
{
	...
	return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[QUESTIONS] WHERE id=$id");
...
function user_exists($id)
{
	...
	return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[USERS] WHERE id=$id");
...
{
	global $db,$DB_TABLES, $OPTIONS;

	$earliest_response = time() - TS_GAME_RESPONSE_TIME_LIMIT;
	$result = $db->query("SELECT id FROM $DB_TABLES[GAME_SESSIONS] WHERE last_response_time < $earliest_response AND end_type=" . TS_GAME_END_TYPE_NOT_ENDED);
	$db->query("UPDATE $DB_TABLES[GAME_SESSIONS] SET end_type=" . TS_GAME_END_TYPE_TIMED_OUT . " 
...
function check_high_score_reset()
{
	...
	$time = time();
	$result = $db->query("SELECT id FROM $DB_TABLES[GAMES] WHERE last_score_reset < $time - (high_score_period * 86400) AND high_score_period != 0");
	while(@list($id) = $result->fetch_array())
		reset_game_scores($id);
...
{
	global $db, $DB_TABLES;
	$db->query("CREATE TABLE IF NOT EXISTS $DB_TABLES[GAME_QUESTION_CACHE]_$game_id (id int unsigned unique primary key)");
	$db->query("INSERT INTO $DB_TABLES[GAME_QUESTION_CACHE]_$game_id SELECT id FROM $DB_TABLES[QUESTIONS], $DB_TABLES[GAME_CATS] WHERE $DB_TABLES[QUESTIONS].category_id=$DB_TABLES[GAME_CATS].category_id AND $DB_TABLES[GAME_CATS].game_id=$game_id ORDER BY id ASC");
...
function delete_game_question_cache($game_id)
{
	...
	$db->query("DROP TABLE IF EXISTS $DB_TABLES[GAME_QUESTION_CACHE]" . "_$game_id");
...
class ts_user
{
	...
	function validate()
	{
		...
		global $db, $DB_TABLES;
		$result = $db->query("SELECT id,username,time_offset,level FROM $DB_TABLES[USERS] WHERE username='$this->username' AND password='$this->password' AND level != 0");
...
class ts_game_session
{
	...
	{
		global $db, $DB_TABLES;
		if( !$result = $db->query("SELECT $DB_TABLES[GAMES].*, $DB_TABLES[GAMES].id AS game_id, $DB_TABLES[GAME_SESSIONS].* 
					FROM $DB_TABLES[GAMES],$DB_TABLES[GAME_SESSIONS] 
					WHERE $DB_TABLES[GAMES].id=$DB_TABLES[GAME_SESSIONS].game_id
					AND $DB_TABLES[GAME_SESSIONS].id=$this->id") )
		{
			return 0;
		}
		if(!$result->num_rows())
	...
	function start()
	{
		global $db, $DB_TABLES, $frontend_link, $C_OPTS;

		$game_question_cache_table = $DB_TABLES[GAME_QUESTION_CACHE] . '_' . $this->game['id'];
		if( $C_OPTS['USE_HEAP_TABLES'] )
		{
			$db->query("CREATE TABLE $DB_TABLES[GSQ]_$this->id type=heap SELECT id, RAND() as RAND FROM $game_question_cache_table ORDER BY RAND LIMIT " . $this->game[questions_per_session]);
		}
		else
		{
			$db->query("CREATE TABLE $DB_TABLES[GSQ]_$this->id SELECT id FROM $game_question_cache_table ORDER BY RAND() LIMIT " . $this->game[questions_per_session]);
		}
	...
	function get_game_info()
	{
		...
		$game_info->high_score_link		= "$OPTIONS[TRIVIA_SITE_URL]/$TS_SCRIPTS[GAME_INFO]?fn=show_high_scores&game_id=" . $this->game[id];
	...
	function fetch_next_question()
	{
		...
		$result = $db->query("SELECT $DB_TABLES[QUESTIONS].* FROM $DB_TABLES[QUESTIONS],$DB_TABLES[GSQ]_$this->id WHERE $DB_TABLES[QUESTIONS].id=$DB_TABLES[GSQ]_" . $this->id . ".id LIMIT $question_offset, 1");
		$row = $result->fetch_array();
		$this->current_question = $row['id'];
		if($this->game[multicategory])
			$category = $db->query_one_result("SELECT name FROM $DB_TABLES[CATEGORIES] WHERE id=$row[category_id]");
		...
		$result = $db->query("SELECT id,answer,answer_order FROM $DB_TABLES[ANSWERS] WHERE question_id=$row[id] ORDER BY RAND()");
		while($row = $result->fetch_array())
		{
	...
	function get_answer_result($answer_id)
	{
		...
		$result = $db->query("SELECT answer_notes FROM $DB_TABLES[QUESTIONS] WHERE id=$this->current_question");
		$question_info = $result->fetch_array();
		$result = $db->query("SELECT id,answer FROM $DB_TABLES[ANSWERS] WHERE question_id=$this->current_question AND correct=1");
	 	$answer_info = $result->fetch_array();
		if( $answer_id == 0 )
		{
	...
	function end($end_type)
	{
		...
		$this->end_time = time();
		$db->query("DROP TABLE IF EXISTS $DB_TABLES[GSQ]_$this->id");
	}
