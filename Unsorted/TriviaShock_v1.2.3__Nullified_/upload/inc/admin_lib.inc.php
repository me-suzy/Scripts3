...
$CP_HVARS = array (
	...
	'TABLE_HEADER_COLOR'		=> '#184A9C',
	'TABLE_HEADER_TEXT_COLOR'	=> '#FFFFFF',

	'TABLE_HEADER2_COLOR'		=> '#4883E4',
...
function show_cp_header($nomargin=0, $body_bgcolor="")
{
	...
	SCROLLBAR-ARROW-COLOR: $CP_HVARS[TABLE_HEADER_COLOR];
	}
	...
	FONT-WEIGHT: bold;
	COLOR: #FFFFFF;
	BACKGROUND-COLOR: $CP_HVARS[TABLE_HEADER_COLOR];
	BORDER: 1 solid
	}
...
function show_cp_footer()
{
	...
	echo "</td></tr></table>";
	echo "<br><br><font size=2 face=\"verdana,arial,tahoma,helvetica\"><center><small>TriviaShock " . TS_VERSION . "<br>";
	echo "Copyright (c)2001-2002 Pineapple Technologies. All Rights Reserved.</small><br><br>";
...
{
	global $db, $DB_TABLES, $ts_user;
	if($ts_user->is_admin($userid))
		return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[CATEGORIES]");
	else
		return $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[CATEGORIES] WHERE user_id=$userid");
...
function get_question_category($id)
{
	...
	return $db->query_one_result("SELECT category_id FROM $DB_TABLES[QUESTIONS] WHERE id=$id");
