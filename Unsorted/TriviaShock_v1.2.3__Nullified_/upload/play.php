...
$result = $db->query("SELECT * FROM $DB_TABLES[GAMES] WHERE id=$game_id");
if( !$result->num_rows() )
{
...
$db->query("INSERT INTO $DB_TABLES[GAME_SESSIONS] (game_id,game_key,user_id,game_type,state,last_response_time) VALUES ($game_id,'$game_key', $user_id, $game_type, " . TS_GAME_STATE_START . "," . time() . ")");
$game_session_id = $db->query_one_result("SELECT id FROM $DB_TABLES[GAME_SESSIONS] WHERE game_id=$game_id AND user_id=$user_id AND state=" . TS_GAME_STATE_START . " AND game_key='$game_key' ORDER BY id DESC LIMIT 1");
...
$play->set("FRONTEND_URL", "$OPTIONS[TRIVIA_SITE_URL]/swf/triviashock.swf?game_session_id=$game_session_id&game_key=$game_key&trivia_site=$trivia_site&game_skin=$game_skin");
