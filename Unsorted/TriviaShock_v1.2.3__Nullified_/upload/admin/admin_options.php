...
function check_input()
{
	...
	$result = $db->query("SELECT * FROM $DB_TABLES[OPTIONS] ORDER BY group_id ASC");
...
function show_form($errors=array())
{
	...
	$result = $db->query("SELECT * FROM $DB_TABLES[OPTION_GROUPS]");
	while($row = $result->fetch_array())
	{
		do_table_header("<b>$row[name]</b>", 2);
		$result2 = $db->query("SELECT * FROM $DB_TABLES[OPTIONS] WHERE group_id=$row[id]");
...
function save_changes()
{
	...
	while(@list($i, $value) = @each($form_options))
	{
		...
		$db->query("UPDATE $DB_TABLES[OPTIONS] SET the_value='$value' WHERE the_key='$i'");
...
function restore_defaults()
{
	...
	$db->query("UPDATE $DB_TABLES[OPTIONS] SET the_value=default_value");
