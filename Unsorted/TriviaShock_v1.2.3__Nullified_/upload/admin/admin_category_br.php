...
function browse_categories()
{
 ...
 global $OPTIONS, $TS_SCRIPTS, $db, $DB_TABLES, $ts_user;
 ...
 $query->add_field("$DB_TABLES[CATEGORIES].*");

 $count_query = new select_query();
 $count_query->add_table($DB_TABLES[CATEGORIES]);
 ...
 else
 {
 	...
 	while($row = $result->fetch_array())
 	{
		...
		start_table_cell($currbg);
		 ...
		 $total_questions = $db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[QUESTIONS] WHERE category_id=$row[id]");
...
function delete_category()
{
 ...
 global $db, $DB_TABLES, $form_options, $query_options;
 ...
 {
	$db->query("DELETE FROM $DB_TABLES[CATEGORIES] WHERE id=$form_options[id]");
	$result = $db->query("SELECT id FROM $DB_TABLES[QUESTIONS] WHERE category_id=$form_options[id]");
	$db->query("DELETE FROM $DB_TABLES[QUESTIONS] WHERE category_id=$form_options[id]");
	while($row = $result->fetch_array())
	{
		$db->query("DELETE FROM $DB_TABLES[ANSWERS] WHERE question_id=$row[id]");
	}
	$db->query("DELETE FROM $DB_TABLES[GAME_CATS] WHERE category_id=$form_options[id]");
