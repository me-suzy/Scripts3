...
show_section_info("Upload Game Skin", "Use this form to upload a Flash 5 Game Skin. Game skins control the look of the game frontend");
...
if(isset($form_options[submit]))
{
	...
	else
	{
		...
		{
			$errors[] = "Invalid file type. The acceptable file type(s) for game skins are:<br><b>" . implode("</b>, <b>", $game_skin_types) . "</b>.";
		}
		if(count($errors))
		{
			show_form($errors);
		...
		else
		{
			...
			{
				error_out("Whoops", "Unable to write game skin file. Please ensure that the game_skins directory is set to writable (chmod 755).");
			}
			
			show_status_message("Uploaded game skin <b>" . $HTTP_POST_FILES[game_skin_file][name] . "</b> successfully.");
...
function show_form($errors=array())
{
	...
	start_form_table();
	do_table_header("<b>Upload Game Skin</b>", 2);

	$currbg = switch_bgcolor($currbg);
	start_table_row();
	do_option_info_cell("Game Skin File", "Select the Flash (.swf) game skin file from your computer.", $currbg);
	...
	do_submitbutton("form_options[submit]", "Upload Game Skin");
	br();
