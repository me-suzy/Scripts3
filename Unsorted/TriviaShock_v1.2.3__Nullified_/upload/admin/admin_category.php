...
{		
	...
	if($form_options[submit])
	{
		if(!($errors = check_input()))
		{
			...
			if($form_options[id] == "new")
			{
				if(!($form_options[id] = $db->insert_from_array($db_stuff,$DB_TABLES[CATEGORIES],"name")))
				{
			...
			{
				$db->update_from_array($db_stuff, $DB_TABLES[CATEGORIES],$form_options[id]);
				unset($db_stuff);
	...
	{
		$form_options = load_data_from_db($DB_TABLES[CATEGORIES], $form_options[id]);
		show_form($errors);
	}
	elseif($form_options[id] == "new")
	{
...
function show_form($errors)
{
 ...
 // if http_post_vars is blank (like it would be in the first run, it must be 
 // a new question
 if(!$form_options[id]) $form_options[id] = "new";
 ...
 // if a new category, make the button say "Create Category"
 if($form_options[id] == "new")
