...
function import_category($tsc_data)
{
	...
	while($db->query_one_result("SELECT COUNT(*) FROM $DB_TABLES[CATEGORIES] WHERE name='$category_name'"))
		$category_name = $tsc_data[category][name] . '_' . ++$x;
	$db->query("INSERT INTO $DB_TABLES[CATEGORIES] (name,description) VALUES ('$category_name', '$category_description')");
	$category_id = $db->query_one_result("SELECT id FROM $DB_TABLES[CATEGORIES] ORDER BY id DESC LIMIT 1");
	...
	for($q=0;$q<$num_questions;$q++)
	{
		...
		$db->query("INSERT INTO $DB_TABLES[QUESTIONS] (question, answer_notes, category_id) VALUES ('$question', '$answer_notes', '$category_id')");
		$question_id = $db->query_one_result("SELECT id FROM $DB_TABLES[QUESTIONS] WHERE category_id='$category_id' ORDER BY id DESC LIMIT 1");
		...
		for($a=0;$a<$num_answers;$a++)
		{
			...
			$db->query("INSERT INTO $DB_TABLES[ANSWERS] (answer, correct, answer_order, question_id) VALUES ('$answer', '$correct', '$answer_order', '$question_id')");
