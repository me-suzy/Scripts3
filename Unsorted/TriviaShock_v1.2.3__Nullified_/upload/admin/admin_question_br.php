...
{
	global $form_questions, $form_options, $query_options, $db, $DB_TABLES, $ts_user, $TS_SCRIPTS;
	...
	$catquery= "SELECT id,name FROM $DB_TABLES[CATEGORIES] ORDER BY name ASC";
	...
	$query->add_table($DB_TABLES['QUESTIONS']);
	$query->add_table($DB_TABLES['CATEGORIES']);
	$query->add_field("$DB_TABLES[QUESTIONS].*");
	$query->add_field("$DB_TABLES[CATEGORIES].name AS category");
 	$query->add_where_clause("$DB_TABLES[QUESTIONS].category_id=$DB_TABLES[CATEGORIES].id");
 
	$count_query = new select_query;
 	$count_query->add_table($DB_TABLES[QUESTIONS]);
	$count_query->add_field("COUNT(*)");
 
	if( $query_options['category_id'] )
	{
		$query->add_where_clause("$DB_TABLES[CATEGORIES].id=$query_options[category_id]");
	...
	if( $query_options['question_search'] )
	{
		...
		$query->add_where_clause("$DB_TABLES[QUESTIONS].question LIKE '%$query_options[question_search]%'");
	
		$count_query->add_where_clause("$DB_TABLES[QUESTIONS].question LIKE '%$query_options[question_search]%'");
	}
	...
	switch( $query_options['sortby'] )
	{
		...
		case 'Category':
		default:
			$query->add_order_by("$DB_TABLES[CATEGORIES].name");
		break;	
	}
	...
	if( !$result->num_rows() )
	{
	 ...
	 else
	 {
		...
		do_select_from_query("form_options[movecategory]", $form_options['movecategory'], "SELECT id,name FROM $DB_TABLES[CATEGORIES] ORDER BY name ASC", "name", "id", "input");
		space();
...
function move_category()
{
 ...
 global $form_options, $form_questions, $db, $DB_TABLES;
 ...
 {
	if($value)
		$db->query("UPDATE $DB_TABLES[QUESTIONS] SET category_id=$form_options[movecategory] WHERE id=$key");
	$qctr++;
			
 } // end while
	
 // Get the name of the category
 $category_name = $db->query_one_result("SELECT name FROM $DB_TABLES[CATEGORIES] WHERE id=$form_options[movecategory]");
...
function change_level()
{
 ...
 {
	if($value)
		$db->query("UPDATE $DB_TABLES[QUESTIONS] SET level_id=$form_options[changelevel] WHERE id=$key");
	$qctr++;
...
function del_question()
{
 ...
 {
	if($value)
		$db->query("DELETE FROM $DB_TABLES[QUESTIONS] WHERE id=$key");
	$qctr++;
