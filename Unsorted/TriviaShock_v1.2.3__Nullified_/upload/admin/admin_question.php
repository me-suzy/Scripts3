...
if( is_valid_id($form_options['id']) )
{
			...
			if($form_options[id] == "new")
			{
				...
				start_table_row();
				 start_table_cell($currbg);
				  ...
				  do_select_from_query("category_id", $form_options[category_id], "SELECT id,name FROM $DB_TABLES[CATEGORIES] ORDER BY name ASC", "name", "id");
				  space();
	...
	{
		$form_options = load_data_from_db($DB_TABLES[QUESTIONS], $form_options[id]);
		$result = $db->query("SELECT * FROM $DB_TABLES[ANSWERS] WHERE question_id=$form_options[id] ORDER BY id ASC");
...
function show_form($errors=array())
{
 ...
 start_table_row();
  ...
  start_table_cell($currbg);
   ...
   $query = "SELECT id,name FROM $DB_TABLES[CATEGORIES] ORDER BY name ASC";
...
function save_changes()
{
 ...
 if($form_options[id] == "new")
 {
 	...
 	$form_options[id] = $db->insert_from_array(array(	'question'	=> $form_options[question],
								...
								'answer_notes'	=> $form_options[answer_notes]), $DB_TABLES[QUESTIONS]);
 } // end if
 ...
 {   
 	$db->update_from_array(array(	'question'	=> $form_options[question],
					'category_id'	=> $form_options[category_id],
					'answer_notes'	=> $form_options[answer_notes]), $DB_TABLES[QUESTIONS], $form_options[id]);
 }
 ...
 while(@list($i, $value) = @each($answers))
 {
	...
	$db->update_from_array(array(	'answer'	=> $answers[$i],
					'correct'	=> $is_correct,
					'answer_order'	=> $answers_order[$i]), $DB_TABLES[ANSWERS], $i);
	$actr++;
 ...
 while(@list($i, $value) = @each($new_answers))
 {
	...
	$answer_id = $db->insert_from_array(array(	'question_id'	=> $form_options[id],
							...
							'correct'	=> $is_correct,
							'answer_order'	=> $new_answers_order[$i]), $DB_TABLES[ANSWERS]);
...
function delete_answers()
{
 ...
 {
	$db->query("DELETE FROM $DB_TABLES[ANSWERS] WHERE id=$i");
	unset($answers[$i]);
...
{
	global $db, $DB_TABLES, $form_options, $ts_user;
