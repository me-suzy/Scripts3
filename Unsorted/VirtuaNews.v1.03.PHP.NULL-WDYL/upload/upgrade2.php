...
$oldversion = "1.0.1 Pro";
$newversion = "1.0.2 Pro";
...
if ($step == 2) {
  ...
  query("CREATE TABLE news_referer (
    name varchar(255) NOT NULL default '',
    value int(10) unsigned NOT NULL default '0',
    PRIMARY KEY  (name))");

  query("CREATE TABLE news_stats (
  ...
  $getstats = query("SELECT name,value FROM news_stat WHERE (name  = 'uniquestotal') OR (name = 'maxusersonline')");
  while ($stats = fetch_array($getstats)) {
    $varname = $stats[name];
    $$varname = $stats[value];
  }

  $morntime = mktime(0,0,0,gmdate('m'),gmdate('d'),gmdate('Y'));
  $morntime = $morntime;

  $uniquestoday = query_first("SELECT COUNT(ip) AS count FROM news_hit WHERE time > $morntime");
  $uniquestoday = $uniquestoday[count];

  query("INSERT INTO news_stats VALUES ('" . time() . "','" . $uniquestotal . "','" . $uniquestoday . "','" . $maxusersonline . "')");

  $getreferers = query("SELECT name,value FROM news_stat WHERE type = 'referer'");

  while ($referer_arr = fetch_array($getreferers)) {
    query("INSERT INTO news_referer VALUES ('" . mysql_escape_string($referer_arr[name]) . "','$referer_arr[value]')");
  }

  query("DELETE FROM news_stat WHERE (type != 'os') AND (type != 'browser')");

  query("ALTER TABLE news_stat RENAME news_useragent");
  query("ALTER TABLE news_useragent ADD KEY (name,type)");
  query("ALTER TABLE news_hit ADD KEY (time,ip)");
  query("ALTER TABLE news_news ADD KEY (program)");
  query("ALTER TABLE news_news ADD KEY (catid)");
  query("ALTER TABLE news_comment ADD KEY (newsid)");
  query("ALTER TABLE news_staff ADD KEY (userid)");
