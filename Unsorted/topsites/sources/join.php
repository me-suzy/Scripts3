...
if (!$FORM['url']) {
  ...
  if ($error_url || $error_email || $error_title || $error_password || $error_urlbanner) {
    ...
    $TMPL['content'] .= "<br />".$LNG['join_error_back'];
  }
  else {
    $test = time() - 120;
    $result = $db->Execute("SELECT jointime FROM ".$CONFIG['sql_prefix']."_members WHERE title = '".$FORM['title']."' AND jointime > ".$test." LIMIT 1");
    list($jointime) = $result->fields;
    if ($jointime) {
    ...
    $file = $CONFIG['templates_path']."/welcome.html";
    ...
    if ($CONFIG['email_admin_on_join']) {
      $file = $CONFIG['templates_path']."/welcome_admin.html";
