...
if($action=="add") {
 ...
 if($usedbtemplates==1) {
  ...
  $tpl = new tpl(0,0);
  $tpl->templatelist="months,timezones,days,register_error1,register_error3,register_error4";
  $tpl->cache();
 }
 else {
 ...
 if(isset($_POST['send'])) {
  ...
  if($error) eval ("\$error = acp_error_frame(\"".gettemplate("users_add_error")."\");");	
   ...
   $db->query("INSERT INTO bb".$n."_users (userid,username,password,email,groupid,rankid,title,regdate,lastvisit,lastactivity,usertext,signature,icq,aim,yim,msn,homepage,birthday,gender,showemail,admincanemail,usercanemail,invisible,usecookies,styleid,activation,daysprune,timezoneoffset,startweek,dateformat,timeformat,emailnotify,receivepm,emailonpm,pmpopup,umaxposts,showsignatures,showavatars,showimages,nosessionhash,avatarid,threadview) 
    VALUES (NULL,'".addslashes(htmlspecialchars($username))."','".md5($password)."','".addslashes(htmlspecialchars($email))."','$groupid','$rankid[rankid]','".addslashes(htmlspecialchars($title))."','".time()."','".time()."','".time()."','".addslashes(htmlspecialchars($usertext))."','".addslashes($signature)."','".intval($icq)."','".addslashes(htmlspecialchars($aim))."','".addslashes(htmlspecialchars($yim))."','".addslashes(htmlspecialchars($msn))."','".addslashes(htmlspecialchars($homepage))."','".addslashes(htmlspecialchars($birthday))."','".intval($gender)."','".intval($showemail)."','".intval($admincanemail)."','".intval($usercanemail)."','".intval($invisible)."','".intval($usecookies)."','".intval($styleid)."','1','".intval($daysprune)."','".addslashes(htmlspecialchars($timezoneoffset))."','".intval($startweek)."','".addslashes(htmlspecialchars($udateformat))."','".addslashes(htmlspecialchars($utimeformat))."','".intval($emailnotify)."','".intval($receivepm)."','".intval($emailonpm)."','".intval($pmpopup)."','".intval($umaxposts)."','".intval($showsignatures)."','".intval($showavatars)."','".intval($showimages)."','".intval($nosessionhash)."','".intval($avatarid)."','".intval($threadview)."')");
 ...
 for($i=0;$i<count($timezones);$i++) {
  ...
  $timezone_options .= makeoption($parts[0],"(GMT".ifelse($parts[1]," ".$parts[1],"").") $parts[2]",$timezoneoffset);
 }
 ...
 if(isset($threadview)) $sel_threadview[$threadview]=" selected";
 
 $y=1;
 $result=$db->query("SELECT * FROM bb".$n."_profilefields ORDER BY fieldorder ASC");
 while($row=$db->fetch_array($result)) {
  $field_value=$field[$row[profilefieldid]];
  $rowclass=getone($y,"firstrow","secondrow");
  eval ("\$userfields .= \"".gettemplate("users_add_userfield")."\";");
  $y++;
 }
...
if($action=="find") {
 ...
 while($row=$db->fetch_array($result)) $group_options.=makeoption($row['groupid'],$row['title'],"",0);	
 $rank_options="";
 $result=$db->query("SELECT rankid, ranktitle FROM bb".$n."_ranks ORDER BY groupid ASC, needposts DESC");
 while($row=$db->fetch_array($result)) $rank_options.=makeoption($row['rankid'],$row['ranktitle'],"",0);	
 $morebit="";
 $count=0;
 $result=$db->query("SELECT profilefieldid, title FROM bb".$n."_profilefields ORDER BY fieldorder ASC");
 while($row=$db->fetch_array($result)) {
  $rowclass=getone(++$count,"firstrow","secondrow");
  eval ("\$morebit .= \"".gettemplate("users_find_morebit")."\";");	
 }
...
if($action=="show") {
 ...
 if(isset($_REQUEST['rankid']) && $_REQUEST['rankid']) add2where("rankid = '".intval($_REQUEST['rankid'])."'");
 if(isset($_REQUEST['title']) && $_REQUEST['title']) add2where("title LIKE '%".addslashes(htmlspecialchars($_REQUEST['title']))."%'");
 if(isset($_REQUEST['usertext']) && $_REQUEST['usertext']) add2where("usertext LIKE '%".addslashes(htmlspecialchars($_REQUEST['usertext']))."%'");
 if(isset($_REQUEST['signature']) && $_REQUEST['signature']) add2where("signature LIKE '%".addslashes($_REQUEST['signature'])."%'");
 	
 if(isset($_REQUEST['homepage']) && $_REQUEST['homepage']) add2where("homepage LIKE '%".addslashes(htmlspecialchars($_REQUEST['homepage']))."%'");
 if(isset($_REQUEST['icq']) && $_REQUEST['icq']) add2where("icq = '".intval($_REQUEST['icq'])."'");
 if(isset($_REQUEST['aim']) && $_REQUEST['aim']) add2where("aim LIKE '%".addslashes(htmlspecialchars($_REQUEST['aim']))."%'");
 if(isset($_REQUEST['yim']) && $_REQUEST['yim']) add2where("yim LIKE '%".addslashes(htmlspecialchars($_REQUEST['yim']))."%'");
 if(isset($_REQUEST['msn']) && $_REQUEST['msn']) add2where("msn LIKE '%".addslashes(htmlspecialchars($_REQUEST['msn']))."%'");
 	
 if(isset($_REQUEST['userposts_morethen']) && $_REQUEST['userposts_morethen']) add2where("userposts > '".intval($_REQUEST['userposts_morethen'])."'");
 if(isset($_REQUEST['userposts_lessthen']) && $_REQUEST['userposts_lessthen']) add2where("userposts < '".intval($_REQUEST['userposts_lessthen'])."'");
 
 if(isset($_REQUEST['lastactivity_in']) && $_REQUEST['lastactivity_in']) add2where("lastactivity >= '".(time()-intval($_REQUEST['lastactivity_in'])*3600)."'");
 if(isset($_REQUEST['lastactivity_notin']) && $_REQUEST['lastactivity_notin']) add2where("lastactivity < '".(time()-intval($_REQUEST['lastactivity_notin'])*3600)."'");
  	
 if(isset($_REQUEST['activation']) && $_REQUEST['activation']==-1) add2where("activation <> '1'");
 elseif(isset($_REQUEST['activation']) && $_REQUEST['activation']==1) add2where("activation = '1'");
 if(isset($_REQUEST['blocked']) && $_REQUEST['blocked']==1) add2where("blocked = '1'");
 if(isset($_REQUEST['gender']) && $_REQUEST['gender']!=0) add2where("gender = '$_REQUEST[gender]'");
 
 $userfields=0;
 if(isset($_REQUEST['profilefield']) && is_array($_REQUEST['profilefield']) && count($_REQUEST['profilefield'])) {
  while(list($key,$val)=each($_REQUEST['profilefield'])) {
   if($val) {
    $userfields=1;
    add2where("field".$key." LIKE '%".addslashes(htmlspecialchars($val))."%'");
   }
  }	
 }
...
if($action=="access") {
 if(isset($_POST['send'])) {
  ...
  if($userids) {
   ...
   while($row=$db->fetch_array($result)) {
    $values="";
    if(!isset($_POST['usedefault'][$row['boardid']]) || !$_POST['usedefault'][$row['boardid']]) {
     $useraccess=1;
     if(isset($_POST['boardpermission'][$row['boardid']])) $boardpermission=$_POST['boardpermission'][$row['boardid']];
     else $boardpermission=0;
     if(isset($_POST['startpermission'][$row['boardid']])) $startpermission=$_POST['startpermission'][$row['boardid']];
     else $startpermission=0;
     if(isset($_POST['replypermission'][$row['boardid']])) $replypermission=$_POST['replypermission'][$row['boardid']];
     else $replypermission=0;
     
     for($i=0;$i<count($userid);$i++) {
      if($values) $values.=",('$row[boardid]','$userid[$i]','$boardpermission','$startpermission','$replypermission')";	
      else $values="('$row[boardid]','$userid[$i]','$boardpermission','$startpermission','$replypermission')";
     }
     $db->unbuffered_query("INSERT INTO bb".$n."_access (boardid,userid,boardpermission,startpermission,replypermission) VALUES $values",1);
    }
   }
   
   if($useuseraccess==0 && $useraccess==1) {
    $db->query("UPDATE bb".$n."_options SET value='1' WHERE varname='useuseraccess'");
 ...
 function makeboardlist($boardid,$x=0) {
  ...
  while (list($key1,$val1) = each($boardcache[$boardid])) {
   while(list($key2,$boards) = each($val1)) {
    ...
    if(isset($permissioncache[$boards['boardid']])) {
     if($permissioncache[$boards['boardid']]['boardpermission']==1) $checked2=" checked";
     if($permissioncache[$boards['boardid']]['startpermission']==1) $checked3=" checked";
     if($permissioncache[$boards['boardid']]['replypermission']==1) $checked4=" checked";
    }
    else $checked1=" checked";
   
    eval ("\$out .= \"".gettemplate("users_accessbit")."\";");
 ...
 if(!$count) eval("acp_error(\"".gettemplate("error_selectnouser")."\");");
 while($row=$db->fetch_array($result)) {
  $userid=$row['userid'];
  if($users) $users.=", ".makehreftag("../profile.php?userid=$row[userid]&sid=$session[hash]",$row['username'],"_blank");
  else $users=makehreftag("../profile.php?userid=$row[userid]&sid=$session[hash]",$row['username'],"_blank");
 ...
 if($count==1) {
  $result=$db->query("SELECT * FROM bb".$n."_access WHERE userid='$userid'");
 ...
 $maxcolspan2=$maxcolspan+4;
  
 eval("print(\"".gettemplate("users_access")."\");");
}
...
if($action=="edit") {
 ...
 if(isset($_POST['send'])) {
  ...
  if($error) eval ("\$error = acp_error_frame(\"".gettemplate("users_add_error")."\");");	
   ...
   $db->unbuffered_query("UPDATE bb".$n."_users SET username='".addslashes($username)."',email='".addslashes(htmlspecialchars($email))."',groupid='$groupid',rankid='$rankid[rankid]',title='".addslashes(htmlspecialchars($title))."',usertext='".addslashes(htmlspecialchars($usertext))."',signature='".addslashes($signature)."',icq='".intval($icq)."',aim='".addslashes(htmlspecialchars($aim))."',yim='".addslashes(htmlspecialchars($yim))."',msn='".addslashes(htmlspecialchars($msn))."',homepage='".addslashes(htmlspecialchars($homepage))."',birthday='".addslashes(htmlspecialchars($birthday))."',gender='".intval($gender)."',showemail='".intval($showemail)."',admincanemail='".intval($admincanemail)."',usercanemail='".intval($usercanemail)."',invisible='".intval($invisible)."',usecookies='".intval($usecookies)."',styleid='".intval($styleid)."',daysprune='".intval($daysprune)."',timezoneoffset='".addslashes(htmlspecialchars($timezoneoffset))."',startweek='".intval($startweek)."',dateformat='".addslashes(htmlspecialchars($udateformat))."',timeformat='".addslashes(htmlspecialchars($utimeformat))."',emailnotify='".intval($emailnotify)."',receivepm='".intval($receivepm)."',emailonpm='".intval($emailonpm)."',pmpopup='".intval($pmpopup)."',umaxposts='".intval($umaxposts)."',showsignatures='".intval($showsignatures)."',showavatars='".intval($showavatars)."',showimages='".intval($showimages)."',nosessionhash='".intval($nosessionhash)."', blocked='".intval($blocked)."', avatarid = '".intval($avatarid)."', threadview='".intval($threadview)."' WHERE userid='$userid'",1);
  ...
  while(list($key,$val)=each($result)) {
   if($key=="dateformat") $udateformat=$val;
   elseif($key=="timeformat") $utimeformat=$val;
   elseif(substr($key,0,5)=="field") $field[intval(substr($key,5))]=$val;
   else $$key=$val;
  }
 ...
 if($ratingcount!=0) $ratingpoints = number_format($ratingpoints/$ratingcount,2); 
...
if($action=="activate") {
 ...
 if($db->num_rows($result)) {
  if($usedbtemplates==1) {
   require("./lib/class_tpl_db.php");
   $tpl = new tpl(0,0);
   $tpl->templatelist="ms_activation,mt_activation";
   $tpl->cache();
  }
  else {
   require("./lib/class_tpl_file.php");
...
if($action=="rate") {
 ...
 eval("print(\"".gettemplate("users_rate")."\");");	
}
 ...
 if(isset($_POST['send'])) {
  ...
  if($_POST['sendmail']==1) {
   if($usedbtemplates==1) {
    require("./lib/class_tpl_db.php");
    $tpl = new tpl(0,0);
    $tpl->templatelist="ms_newpw,mt_newpw";
    $tpl->cache();
   }
   else {
    require("./lib/class_tpl_file.php");
...
if($action=="selectstats") {
 ...
 eval("print(\"".gettemplate("users_stats_select")."\");");	
}

if($action=="showstats") {
 $userids=trim($_POST['userids']);
 if(!$userids) eval("acp_error(\"".gettemplate("error_selectnouser")."\");");
