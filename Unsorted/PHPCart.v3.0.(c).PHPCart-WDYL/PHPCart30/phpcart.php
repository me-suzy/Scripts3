...
if ($action) {
	...
	if ($action=="confirm" || $action=="checkout") {
		print "	<CENTER><FORM ACTION=\"phpcart.php\" METHOD=\"post\" TARGET=\"_self\">
			...
			<TH COLSPAN=2 BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\">$PersonalInfoWord</FONT></TH>
			</TR><TR><TD BGCOLOR=$rowsColor> ";
		if (empty($name) and !empty($alert)) {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"Red\"><B>$NameWord</B></FONT>";
		} else {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$NameWord</B></FONT>";
		}
		...
		if (empty($email) and !empty($alert)) {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"red\"><B>$EmailWord</B></FONT>";
		} else {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\" COLOR=\"$TextColor\"><B>$EmailWord</B></FONT>";
		}
		print "	</TD><TD BGCOLOR=$rowsColor>
		...
		if (empty($address) and !empty($alert)) {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"red\"><B>$deliveryAddress</B></FONT>";
		} else {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$deliveryAddress</B></FONT>";
		}
		print "	</TD><TD BGCOLOR=$rowsColor>
		...
		if ((empty($postcode0)) and !empty($alert)) {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"red\"><B>$postcode</B></FONT>";
		} else {
			print "<FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$postcode</B></FONT>";
		}
		print "	</TD><TD BGCOLOR=$rowsColor>
...
// ========================
	...
	if($action=="add" || $action=="confirm" || $action=="checkout" || $action=="view") {
		print "	<TR BGCOLOR=\"$headerColor\" width=\"1\">
			<TH COLSPAN=2><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$IDWord</B></FONT></TH>
			<TH><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$descriptionWord</B></FONT></TH>
			<TH><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$productPriceWord &nbsp;</B></FONT></TH>
			<TH><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$quantityWord</B></FONT></TH>
			<TH><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$sumPriceWord &nbsp;</B></FONT></TH></TR> ";
		$line_no = 1;
		$tracker = 4;
		while ($data = fgetcsv ($fp, 500)) {
			$num = count ($data);
			print "	<TR BGCOLOR=$rowsColor>
				<TD><A HREF=\"phpcart.php?action=delete&id=$line_no\"><IMG SRC=\"./images/trashicon.gif\" BORDER=0 WIDTH=16 HEIGHT=16></A></TD>
	...
	if ($action=="add" || $action=="view") {
		print "	<TR BGCOLOR=$rowsColor><TD BGCOLOR=\"$PgBack\" COLSPAN=5>                <p align=right></TD><TD>
	 	...
	 	print "	<TR>
			<TD COLSPAN=3 ALIGN=left ROWSPAN=5><CENTER><A HREF=\"$home\"><IMG SRC=\"./images/$backPicture\" BORDER=0></A></CENTER><BR><B><SMALL>$extrasText</SMALL></B><BR></TD>
			<TD COLSPAN=2 ALIGN=right BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$SubTotalWord</B></FONT></TD>
			<TD BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$currency$tot_pos &nbsp;</B></FONT></TD>
			</TR>";
	if (!empty($salesVAT)) {
		print "	<TR>
			<TD COLSPAN=2 ALIGN=right BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$VATWord</B></FONT></TD>
			<TD BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$currency$vat &nbsp;</B></FONT></TD>
			</TR>";
	} else {
	 	print " ";
	}
	if (!empty($PostalAmount)) {
		print "	<TR>
			<TD COLSPAN=2 ALIGN=right BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$PostageWord</B></FONT></TD>
	        	<TD BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$currency$postage &nbsp;</B></FONT></TD>
	        	</TR>";
	} else {
	 	print " ";
	}
	 	print "	<TR>
		        <TD COLSPAN=2 ALIGN=right BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$TotalSumWord</B></FONT></TD>
			<TD BGCOLOR=\"$headerColor\"><FONT FACE=\"$font\" SIZE=$fontSize COLOR=\"$TextColor\"><B>$currency$total &nbsp;</B></FONT></TD>
			</TR><TR>
