...
// Figure out if the SQL server supports AUTO INCREMENT
  ...
  echo "Adding a signature display for users...<br>";
  $query = " 
    ALTER TABLE {$config['tbprefix']}Users
    ADD   U_ShowSigs $varchar(3)
  ";
  $dbh -> do_query($query);

  echo "Adding fields to store last poster on the main page...<br>";
  $query = " 
    ALTER TABLE {$config['tbprefix']}Boards
    ADD   Bo_Poster $varchar(64)
  ";
  $dbh -> do_query($query);
  ...
  while (list($Board) = $dbh -> fetch_array($sth)) {
     ...
     list($Poster) = $dbh -> fetch_array($sti);
     $Poster_q = addslashes($Poster);
     $query = "
        UPDATE {$config['tbprefix']}Boards
        SET    Bo_Poster = '$Poster_q'
