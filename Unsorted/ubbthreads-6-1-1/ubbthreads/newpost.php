...
// Require the library
   ...
   require ("languages/${$config['cookieprefix']."w3t_language"}/newpost.php");

// -----------------
// Get the user info
   $userob = new user;
   $user = $userob -> authenticate("U_EReplies,U_TextCols,U_TextRows,U_Preview,U_Groups, U_Display");
...
// --------------------------------------------------------------
// Let's make sure they are supposed to be making posts here 
...
// No options, we use the board default
   ...
   if ( ($config['files']) && ($Reged == "y") && (ini_get(file_uploads)  ) ) {
	...
	if (!$debug) {
   	include("$thispath/templates/$tempstyle/newpost.tmpl");
	}
   $html -> send_footer();
