...
// Display page to add a new post icon
   ...
   echo "
    <tr class=\"darktable\">
    <td colspan=\"3\">
The following graemlins are in the system.  You may choose any of these to delete and they will be removed from the database and the graemlins directory.
    </td>
    </tr>
    <FORM METHOD = POST action =\"{$config['phpurl']}/admin/doremovegraemlin.php\">
...
";
	 ...
	 while (list($number,$image) = $dbh -> fetch_array($sth)) {
		echo "<input type=\"checkbox\" name=\"$number\" value=\"$number\"> <img src=\"{$config['images']}/graemlins/$image\" /> &nbsp; &nbsp; &nbsp;";
      $i++;
      if ($i==5) {
         echo "<br />";
         $i=0;
	...
	echo "
      ...
      <tr class=\"darktable\">
        <td colspan=\"3\">
          <input type=\"submit\" value=\"Remove selected\" class=\"buttons\" />
   ";
   $html -> close_table();
   echo"</form>";
