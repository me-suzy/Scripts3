...
// ---------------------------------------------------
// Let's see if the database password has been changed
	if ($dbpass != "hiddenpass") {
		$dbpass = $dbpass;
	}
	else {
...
// the group.  If it was unchecked we remove them from it.
   ...
   while ( list($Name,$Id) = $dbh -> fetch_array($sth)) {
      ...
      $which = "GROUP$Id";
      if ($HTTP_POST_VARS{$which}) {
         $newusergroup .= $HTTP_POST_VARS{$which} ."-";
      }
   }
...
*/
  ...
  // manipulating their referer variable.
   \$config['disablerefer'] =	"$disablerefer";
  ...
  // DOING
    \$config['newusergroup'] = "$newusergroup";
  ...
  // Title of Homepage link
    \$config['urltitle']=	"$urltitle";

  // Email address
    \$config['emailaddy']=	"$emailaddy";

  // EMail Title
    \$config['emailtitle']=	"$emailtitle";

  // Show privacy statement link in footer
    \$config['privacy'] = "$privacy";

  // Show board rules on new user screen?
    \$config['boardrules'] = "$boardrules";
  ...
  // 0 = No : 1 = Yes
    \$config['markupoption'] = $markupoption;

  // Do you want users to be able to mail posts to others
  // 0 = No ; 1 = Yes
    \$config['mailpost'] =  $mailpost;
  ...
  // Allow the image markup tag.
    \$config['allowimages']=	$allowimages;
...
// Write the new config
   ...
   $fd = fopen("$configdir/config.inc.php","w");
   fwrite($fd,$newconfig);
   fclose($fd);

// ------------------------
// Send them a confirmation
  $html -> send_header ("config.inc.php updated",$Cat,"<META HTTP-EQUIV=\"Refresh\" content=\"5;url={$config['phpurl']}/admin/main.php?Cat=$Cat\">",$user);
  ...
  echo "The configuration file, config.inc.php, has been updated";
