...
// Grab the board info 
   ...
   $query = "
    SELECT B_Number,B_Main,B_Subject,B_Username,B_Posted,B_Kept 
    FROM  {$config['tbprefix']}Posts 
    WHERE B_Topic = 1 AND B_Kept = '$Closed_q'
...
// ------------------------
// Send them a page 
   $html -> send_header ("Unkeep threads",$Cat,0,$user);
   $html -> admin_table_header("Unkeep threads");
   $html -> open_admin_table();

   echo "
    <FORM METHOD = POST action =\"{$config['phpurl']}/admin/dounkeepthreads.php\">
    <INPUT TYPE=HIDDEN NAME=Cat VALUE=$Cat>   
    <input type=hidden name=Keyword value=$Keyword>

    <TR><TD COLSPAN=4 class=\"lighttable\">
    Place a checkmark by any threads you wish to unkeep. 
    </TD></TR>
    <tr><td class=\"tdheader\">Unkeep</td> 
   ...
   echo "<TR><TD class=\"lighttable\" COLSPAN=4>";
