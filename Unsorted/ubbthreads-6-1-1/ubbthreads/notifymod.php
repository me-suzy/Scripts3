...
// Get the user info
   $userob = new user;
   $user = $userob -> authenticate("U_Email,U_Groups");
   $Username = $user['U_Username'];
   $html = new html;

// -----------------------------------------
// require the language file for this script
   require "languages/${$config['cookieprefix']."w3t_language"}/notifymod.php";
...
// Once and a while if people try to just put a number into the url, lets trap it
   if (!$Number) {
      $html -> not_right($ubbt_lang['NO_POST'],$Cat);
   }
...
// Now let's see if the mod has already been notified
   $query = "
    SELECT M_Number
    FROM   {$config['tbprefix']}ModNotify
    WHERE  M_Number = '$Number'
   ";
   $sth = $dbh -> do_query($query);
   list($check) = $dbh -> fetch_array($sth); 
   if ($check) {
      $html -> not_right("{$ubbt_lang['NO_NOTIFY']}",$Cat);
   }
 
// ---------------
// Send the header
   $html -> send_header($ubbt_lang['NOTIFY_MOD'],$Cat,0,$user);

	if (!$debug) { 
   	include("$thispath/templates/$tempstyle/notifymod.tmpl"); 
	}
