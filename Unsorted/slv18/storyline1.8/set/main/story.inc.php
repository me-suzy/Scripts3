...
<?
	if ($table[0]["sadd"] != 1) {
		print "<a href='addto.php?no=" . $table[0]["sid"] . "''>";
		print "<img src='" . SL_ROOT_URL . "/base/html/Default/images/0.gif' border='0' alt='Add chapter to " . $table[0]["sname"] . "'></a>";
	}
?>
					</td>
					<td width='15%'>
						Author: <a href='authors.php?no=<?=$table[0]["uid"]?>'><?=$table[0]["upenname"]?></a>
					</td>
					<td width='15%'>
						Hits: <?=$table[0]["sthits"] = $table[0]["sthits"]+1?>
					</td>
				</tr><tr>
					<td>
						<?=$cl->prevb($table[0]["cbody"])?>
					</td>
				</tr>
			</table>
			</form>
...
$dl->update("sl18_stories",array('sthits'=>$table[0]["sthits"]),array('sid'=>$_GET["no"]));
}
