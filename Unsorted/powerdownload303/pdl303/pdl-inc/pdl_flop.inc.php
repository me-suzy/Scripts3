<?
$release_res = $db_handler->sql_query("SELECT SUM(downloads) AS downloads, SUM(size) AS size, release.* FROM $sql_table[files] AS files, $sql_table[release] AS release WHERE release.release_id=files.release_id AND release.released='Y' GROUP BY release_id ORDER BY downloads ASC LIMIT 0,$settings[top_count]");
$count = 0;
...
echo replace($template[flop_box], $release_rows);
?>
