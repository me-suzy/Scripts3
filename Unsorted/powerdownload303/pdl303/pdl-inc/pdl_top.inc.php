<?
$release_res = $db_handler->sql_query("SELECT SUM(downloads) AS downloads, SUM(size) AS size, release.* FROM $sql_table[files] AS files, $sql_table[release] AS release WHERE release.release_id=files.release_id AND release.released='Y' GROUP BY release_id ORDER BY downloads DESC LIMIT 0,$settings[top_count]");
$count = 0;
$total = $db_handler->sql_num_rows($release_res);

$usetext = false;
if(preg_match("/\{text\}/",$template[top_row])) $usetext = true;
...
echo replace($template[top_box], $release_rows);
?>
