...
<html>
<head>
<title>Submit New Classified Ad</title>
...
<?
		...
		isset($Description) && (strcmp($Category, "None") != 0) )
	{
		...
		$query = "SELECT * FROM Ads WHERE UserAcctID=$UserAcctID AND CatID=$Category AND ";
		$query .= "Title='$Title' AND Description='$Description' AND Price='$Price' AND Quantity='$Quantity'";
		...
		// insert the ad
    		...
    		$TimeNow = time();
		$EndTime = time() + $AdExpiration;		

		$IP = getenv("REMOTE_ADDR"); 

		$query = "INSERT INTO Ads (UserAcctID, CatID, Title, Description, Price, Quantity, InitialAdDate, BeginDate, EndDate, CreatedIPAddress, ModifiedIPAddress) ";
		$query .= "VALUES ($UserAcctID, $Category, '$Title', '$Description', '$Price', $Quantity, $TimeNow, $TimeNow, $EndTime, '$IP', '$IP')";
		$result = mysql_query( $query, $link );		
