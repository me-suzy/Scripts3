...
if ($number == 0)
{
	make_error_page(!check_emailformate($sender_email),$MsgErrorNotFoundTxt);
	exit;
}
...
$bbcode_card_message = parse_vcode($card_message);
...
if(!empty($card_template))
{
	...
	$t->set_var("SOUND_INFO"	,$musicinfo);
...
if(eregi('.html',$card_file))
{
	...
	eval("make_output(\"".get_template("printable")."\");");
}
