...
/* get bids for current item */
   $query       = "select max(bid) as maxbid from ".$dbfix."_bids where auction=\"$auction_id\" group by auction";
   ...
   {
		/* Get number of bids  */
		$query          = "select * from ".$dbfix."_bids where auction=\"$auction_id\"";
        ...
        $TPL_vendetor_value = " <A HREF=\"profile.php?auction_id=".$auction_id."&user_id=".$user_id."\">".$TPL_user_nick_value."</A>";
       ...
       if ( $high_bidder_id ){
          $TPL_hight_bidder_id  = "<TR><TD WIDTH=\"30%\"><FONT FACE=\"Verdana,Arial, Helvetica, sans-serif\" SIZE=\"2\"><B>"
                                  ...
                                  ."<A HREF=\"profile.php?auctuon_id=$auction_idid&user=$high_bidder_nick\">$high_bidder_nick</A></FONT>";
		...
		$TPL_hight_bidder_id = 
							    ...
							    "<TD WIDTH=\"30%\">".
   								...
   								"</FONT>".
   								"</TD> </TR>";
//			"El maximo oferente: <IMG SRC=\"./images/estrella_".$rate_ratio.".gif\" ALIGN=TOP>";
		
		//$TPL_hight_bidder_id = "<IMG SRC=\"./images/estrella_".$rate_ratio.".gif\" ALIGN=TOP>";
   }
   ...
   $TPL_currency_value4 = $currency." ".number_format($next_bid,0,",",".");
...
//-- Display bids history               
				...
				include ("./templates/bidhistory_php3.html");
               	...
               	while  ( $i < $num_bids )
				{
                    ...
                    $query = "select id,nick from ".$dbfix."_users where id=\"$bidder\"";
                    $result_bidder = mysql_query($query);
                    if(!$result)
					{
						print $ERR_001;
