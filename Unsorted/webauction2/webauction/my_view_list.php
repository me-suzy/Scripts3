...
$rsl = mysql_query ( "SELECT count(*) FROM ".$dbfix."_views WHERE user='".addslashes($user_id)."' ORDER BY ends");
                                                        if ($rsl)
                                                        {
        ...
        /* get active auctions for this user */
                        $result = mysql_query ( "SELECT * FROM ".$dbfix."_views WHERE user='".addslashes($user_id)."'  ORDER BY ends LIMIT $left_limit,$lines" );
                                ...
                                while ($row=mysql_fetch_array($result))
                                {
                                        ...
                                        $tplv .= "<TR ALIGN=CENTER VALIGN=MIDDLE BGCOLOR=\"$bgColor\">";
                                                ...
                                                /* this subastas title and link to details */
                                                        ...
                                                        $tplv .=
                                                                "<TD ALIGN=LEFT><A HREF=\"item.php?id=".$row[auction]."\"><FONT FACE=\"Arial\" color=\"$font_color\" SIZE=2>".
 ...
 /* Abfrage für current_bid ENDE  */
       ...
       $year          = intval ( date("Y"));
        ...
        $ends_seconds  = substr ( $ends, 12, 2 );
                                ...
                                $r_difference = $difference = mktime($ends_h,$ends_min,$ends_sec,$ends_m,$ends_d,$ends_y)-time();
                                                                         if ( intval($r_difference/3600)>24 )
                                                        ...
                                                        $tplv .= "<TD><FONT color=\"$font_color\" FACE=\"Arial\" SIZE=2><b><font color=#FF0000>E-Mail wurde versand</font></b></TD>";




                                                        $tplv .= "<TD><form action=del_views.php method=POST>".
                            ...
                            "<INPUT TYPE=HIDDEN NAME=\"TPL_password\" VALUE=\"$TPL_password\">".
                                        ...
                                        for ($i=1; $i<=$pages; ++$i)
                                        {
                                                $TPL_auctions_list_value .=
                                                        ($page==$i)        ?
                                                                " $i "        :
                                                                " <A HREF=\"my_view_list.php?SESSION_ID=$SESSION_ID&user_id=$user_id&TPL_nick=$TPL_nick&TPL_password=$TPL_password&name=$name&page=$i&action=0\"><font size=\"2\" face=\"arial\" color=\"#000000\">$i</A> ";
                                        }
                                        ...
                                        face=arial color=#000000> Keine Auktionen in der Beobachtungsliste 
                                        vorhanden.</FONT>"; 
                                        }
