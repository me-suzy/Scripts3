...
$rsl = mysql_query ( "SELECT count(*) FROM ".$dbfix."_auctions WHERE user='".addslashes($user_id)."' AND closed='0' and current_bid='0.0000'");
                                                        if ($rsl)
                                                        {
        ...
        /* get active auctions for this user */
                        $result = mysql_query ( "SELECT * FROM ".$dbfix."_auctions WHERE user='".addslashes($user_id)."' AND closed='0' AND current_bid='0.0000' ORDER BY ends LIMIT $left_limit,$lines" );
                                ...
                                while ($row=mysql_fetch_array($result))
                                {
                                        ...
                                        $tplv .= "<TR ALIGN=CENTER VALIGN=MIDDLE BGCOLOR=\"$bgColor\">";
                                                ...
                                                /* this subastas title and link to details */
                                                        ...
                                                        $tplv .=
                                                                "<TD ALIGN=LEFT><A HREF=\"my_active_auctions_change_item.php?SESSION_ID=$SESSION_ID&user_id=$user_id&name=$name&nick=$TPL_nick&password=$TPL_password&id=".$row[id]."\"><FONT color=\"$font_color\" FACE=\"Arial\" SIZE=2>".
                                                ...
                                                /* current bid of this subastas */
                                                      ...
                                                      $row[minimum_bid] = substr($row[minimum_bid],0,2);
                                                      $row[reserve_price] = substr($row[reserve_price],0,2);
                                                                ...
                                                                $tplv .=
                                                                                ...
                                                                                "</TD><TD ALIGN=RIGHT><font face=arial size=2 color=$font_color>".
                                                                                $row["minimum_bid"]."00 $currency</TD></TR></TABLE>".
                                                                                "</TD>";
                                                ...
                                                /* number of bids for this subastas */
                                                                ...
                                                                if ($rpr!=0)
                                                                        ...
                                                                        $reserved_price = "";
                                                        $tplv .= "<TD><FONT color=\"$font_color\" FACE=\"Arial\" SIZE=2>".$num_bids." Gebote</TD>";
                                                ...
                                                /* time left till the end of this subastas */
                                                        ...
                                                        $tplv .= "<TD><FONT color=\"$font_color\" FACE=\"Arial\" SIZE=2>$days_difference Tage <BR>$hours_difference:$minutes_difference:$seconds_difference h</TD>";
                                        ...
                                        for ($i=1; $i<=$pages; ++$i)
                                        {
                                                $TPL_auctions_list_value .=
                                                        ($page==$i)        ?
                                                                " $i "        :
                                                                " <A HREF=\"my_active_auctions_change.php?SESSION_ID=$SESSION_ID&user_id=$user_id&TPL_nick=$TPL_nick&TPL_password=$TPL_password&name=$name&page=$i&action=0\"><font size=\"2\" face=\"arial\" color=\"#000000\">$i</A> ";
                                        }
                                        ...
                                        face=arial color=#000000> Es sind keine unbebotenen Auktionen
                                        vorhanden.</FONT>"; 
                                        }
        ...
        /* get this user's nick */
                        ...
                        include "header.php";
                        include "templates/active_auctions_change_header_php3.html";
                        include "templates/auctions_active_change.html";
                        include "footer.php";
                        exit;
?>
