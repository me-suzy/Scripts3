...
*/
        ...
        if(($sofortkauf > 1)&($current_bid < 1)){
            $Tpl_Sofort = "<IMG SRC=\"./images/sofort.gif\" BORDER=0><form action=\"./sofortkauf.php?id=$id&sofortkauf=$sofortkauf\">
$sofortkauf $currency 
<input type=\"button\" name=\"Sofortkaufen\" value=\"Sofort kaufen\" onClick=\"self.location.href='./sofortkauf.php?id=$id&sofortkauf=$sofortkauf'\">
</p>
</form>";
...
/*        } */
        ...
        $result=mysql_query("SELECT * FROM ".$dbfix."_aufrufzaehler WHERE auktionsid ='$id'");

                        if (mysql_num_rows($result)==0)
                        {
                        $result=mysql_query("INSERT INTO ".$dbfix."_aufrufzaehler (auktionsid,zugriff) VALUES('$id','1')");
                        $zugriff=1;
                        }
                        else
                        {
                        ...
                        $result=mysql_query("UPDATE ".$dbfix."_aufrufzaehler SET zugriff=zugriff+1 WHERE auktionsid='$id'");
                        }
                        $stellen = strval($zugriff);
                        for ($i = 0; $i < strlen($zugriff); $i++) {
                        $counter=$counter."<img src=images/$stellen[$i].jpg >";
                        }
        ...
        if ( $phu!=0 )
//		$pict_url = $image_upload_path.$pict_url;
        ...
        {
           $increment = getIncrement($high_bid);
   ...
   if ( $pict_url )
    ...
    if ($difference > 0) {
        ...
        $TPL_currency_value1 = number_format($minimum_bid,0,",",".")." ".$currency;
        $TPL_currency_value2 = number_format($high_bid,0,",",".")." ".$currency;
        $TPL_currency_value3 = number_format($increment,0,",",".")." ".$currency;
        $TPL_currency_value4 = number_format($next_bid,0,",",".")." ".$currency;
