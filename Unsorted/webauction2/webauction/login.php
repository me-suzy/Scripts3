...
{
	include "header.php";	
	include "templates/user_login_php3.html";
}
...
if ($REQUEST_METHOD=="POST" && $action=="login") 
{
	...
	{
		//$TPL_password = md5($TPL_password);
		$sql="SELECT * FROM".$dbfix."_ users WHERE nick=\"". AddSlashes($TPL_nick)."\"";
		$res=mysql_query ($sql);
		if ($res) 
		{
			if (mysql_num_rows($res)>0) 
			{
               ...
               setcookie("YA_USER_NICK",$TPL_nick,$expires,'','',0);					
					...
					header ("location:  einrichten_ver.php?SESSION_ID=$sessionIDU&id=0&user=".$TPL_nick."&password=".$TPL_password."&zip=".$TPL_zip."&country=".$TPL_country."&name=".$TPL_name."");
				}
				else 
				{
					$TPL_err=1;
...
if ($REQUEST_METHOD=="POST" && $action == "update" && $TPL_err) 
{
	...
	header ("location:  einrichten_ver.php?id=0&user=".$TPL_nick."&password=".$TPL_password."&zip=".$TPL_zip."&country=".$TPL_country."&name=".$TPL_name."");
}
