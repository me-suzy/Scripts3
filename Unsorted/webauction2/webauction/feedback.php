...
if ($REQUEST_METHOD=="POST") 
{
	if ($TPL_rater_nick && $TPL_password && $TPL_rate && $TPL_feedback) 
	{
		...
		{
			$sql="SELECT nick, password FROM ".$dbfix."_users WHERE nick=\"" .AddSlashes($TPL_rater_nick)."\"";
			$res=mysql_query ($sql);
			if (mysql_num_rows($res)>0) 
			{
				...
				{
					$secid = AddSlashes($id);
					$sql="SELECT rate_sum, rate_num FROM ".$dbfix."_users WHERE id='$secid'";
					$res=mysql_query ($sql);
					if ($res) 
					{
						$arr=mysql_fetch_array ($res);
						...
						$sql="UPDATE ".$dbfix."_users SET rate_sum='$secratesum', rate_num='$secratenum' WHERE id='$secid'";
						mysql_query ($sql);
...
{
	$secid = AddSlashes($id);
	$sql="SELECT nick, rate_sum, rate_num FROM ".$dbfix."_users WHERE id='$secid'";
	$res=mysql_query ($sql);
	if ($res) 
	{
		if (mysql_num_rows($res)>0) 
		{
			$arr=mysql_fetch_array ($res);
			$sql="SELECT * FROM ".$dbfix."_feedbacks WHERE rated_user_id='$secid' ORDER by date DESC";
...
if ($REQUEST_METHOD=="GET" && $faction=="show"){
	include "header.php";	
	include "templates/show_feedback.html";
	include "footer.php";
}
