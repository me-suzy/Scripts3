...
$rsl = mysql_query ( "SELECT count(*) FROM ".$dbfix."_auctions WHERE user='".addslashes($user_id)."' AND (closed='1' or closed='del') AND current_bid>='1.00' ORDER BY ends");
                                                        if ($rsl)
                                                        {
        ...
        /* get active auctions for this user */
                        $result = mysql_query ( "SELECT * FROM ".$dbfix."_auctions WHERE user='".addslashes($user_id)."' AND (closed='1' or closed='del') AND current_bid>='1.00' ORDER BY ends LIMIT $left_limit,$lines" );
                                ...
                                while ($row=mysql_fetch_array($result))
                                {
                                        ...
                                        $tplv .= "<TR ALIGN=CENTER VALIGN=MIDDLE BGCOLOR=\"$bgColor\">";
                                                ...
                                                /* this subastas title and link to details */
                                                        ...
                                                        $tplv .=
                                                                "<TD ALIGN=LEFT><A HREF=\"item2.php?id=".$row[id]."\"><FONT color=\"$font_color\" FACE=\"Arial\" SIZE=2>".
                                        ...
                                        for ($i=1; $i<=$pages; ++$i)
                                        {
                                                $TPL_auctions_list_value .=
                                                        ($page==$i)        ?
                                                                " $i "        :
                                                                " <A HREF=\"my_sells1.php?SESSION_ID=$SESSION_ID&user_id=$user_id&TPL_nick=$TPL_nick&TPL_password=$TPL_password&name=$name&page=$i&action=0\"><font size=\"2\" face=\"arial\" color=\"#000000\">$i</A> ";
                                        }
                                        ...
                                        if ($auctions_count==0)
                                        {
                                                $TPL_auctions_list_value = "<table width=\"745\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><TR ALIGN=CENTER><TD COLSPAN=5><br><font size=\"2\" face=\"arial\" color=\"#000000\"> Es sind keine Verkäufe vorhanden.</FONT>";
                                        }
        ...
        /* get this user's nick */
                        ...
                        include "header.php";
                        include "templates/users_auctions_header_sells1_php3.html";
