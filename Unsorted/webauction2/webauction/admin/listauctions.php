...
<TD>
	...
	<TR>
	 <TD ALIGN=CENTER COLSPAN=5>
		<BR>
		<B><? print $tlt_font.$MSG_067; ?></B>
		<BR>
	 </TD>
	</TR>
	<?
		$query = "select count(id) as auctions from ".$dbfix."_auctions";
	...
	<TR BGCOLOR="#32972d">
	  ...
	  <?
		$query = "select a.id, u.nick, a.title, a.date, a.description, c.cat_name, d.description as duration, a.suspended from ".$dbfix."_auctions a, ".$dbfix."_users u, ".$dbfix."_categories c, ".$dbfix."_durations d where u.id = a.user and c.cat_id = a.category and d.days = a.duration order by nick limit $offset, $limit";
		$result = mysql_query($query);
		if(!$result){
		...
		while($i < $num_auctions){
			...
			$tmp_date = mysql_result($result,$i,"date");
