...
*/
   ...
   if (!$authenticated) {
	...
	include "./header.php";
        ...
        if ($act == "delete") {
		$query = "DELETE FROM ".$dbfix."_news WHERE topic = '" . $topic . "';";
...
<TD>
	...
	<TR BGCOLOR="#32972d">
	  ...
	  <?
		$query = "select topic,newstext from ".$dbfix."_news order by topic limit $offset, $limit";
		$result = mysql_query($query);
		if(!$result){
		...
		while($i < $num_topics){
			...
			print "<TR BGCOLOR=$bgcolor>
					<TD>
						...
						<TD ALIGN=LEFT>
						<A HREF=\"addnews.php?act=edit&topic=$topic&offset=$offset\" class=\"nounderlined\">$MSG_298</A><BR>
						<A HREF=\"listnews.php?act=delete&topic=$topic&offset=$offset\" class=\"nounderlined\">$MSG_299</A><BR>
						<TR>";

			$i++;
		}
		print "<TR><TD COLSPAN=3><BR><CENTER><A HREF=\"addnews.php\">$MSG_917</A></CENTER><BR>";
		print "</TABLE></FONT>";
		...
		while($i < $num_pages ){
			...
			if($of != $offset){
				print "<A HREF=\"listnews.php?offset=$of\" CLASS=\"navigation\">".($i + 1)."</A>";
