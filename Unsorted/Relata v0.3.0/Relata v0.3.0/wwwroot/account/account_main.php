...
$template->set_var(array(
	...
	"LOADER"		=>	'<iframe src="' . $_PHPLIB['webdir'] . 'account/account_loader.php?session_id=' . $session_id . '&account_id='.$account_id.'" name="loader" width="0" height="0"></iframe>',
	"ACTION2"		=>	"submit",
	"ACTION2_LABEL"	=>	"Delete",
	"FORM_URL"		=>	$_PHPLIB['webdir'] . "account/account_loader.php?session_id=$session_id"
	));
...
if($action == "add")
{
	...
	$misc_javascript .= "parent.curbtn = $account_id";
	$template->set_var("RELOAD",$misc_javascript);
}

$user_id = $auth->auth["uid"];

$template->set_var("CONTACTS",$avail_contacts);
