...
define("PATH_TO_VB", "http://example.com/forums");
...
function get_users_playing($game_id=0)
{
	...
	if($game_id)
	{
		...
		return $db->query("SELECT user.username, user.userid AS id
					FROM user, $DB_TABLES[GAME_SESSIONS] 
					WHERE user.userid = $DB_TABLES[GAME_SESSIONS].user_id 
	...
	else
	{
		...
		return $db->query("SELECT $DB_TABLES[GAMES].name AS game_name, user.username
					FROM $DB_TABLES[GAMES], $DB_TABLES[GAME_SESSIONS], user
					WHERE $DB_TABLES[GAMES].id = $DB_TABLES[GAME_SESSIONS].game_id 
					AND $DB_TABLES[GAME_SESSIONS].user_id = user.userid
...
function get_high_scores($game_id, $num_high_scores)
{
	...
	return $db->query("SELECT user.username, user.userid AS id, $DB_TABLES[GAME_SESSIONS].score, $DB_TABLES[GAME_SESSIONS].end_time
				FROM user, $DB_TABLES[GAME_SESSIONS] 
				WHERE user.userid = $DB_TABLES[GAME_SESSIONS].user_id 
				AND $DB_TABLES[GAME_SESSIONS].game_id=$game_id 
...
function increment_games_played($user_id)
{
	...
	if( $profile_field_id = $db->query_one_result("SELECT the_value FROM $DB_TABLES[BLACKBOARD] WHERE the_key='VB_PROFILE_FIELD_ID_GAMES_PLAYED'") )
	{
...
class ts_user
{
	...
	function validate()
	{
		...
		if( $this->username )
		{
			...
			$result = $db->query("SELECT userid,usergroupid,username,timezoneoffset FROM user WHERE username='$this->username' AND password='$this->password'");
		}
		
		// else validate from user_id (cookie)
		elseif( $this->user_id )
		{		
			$result = $db->query("SELECT userid,usergroupid,username,timezoneoffset FROM user WHERE userid='$this->user_id' AND password='$this->password'");
		}
