<?
	...
	if($del_id)
	{
		$query = "select member_id from dt_profile where id = '$del_id'";
		$row = f(q($query));
		...
		{
			$query = "SELECT members.login as login, profile.id as id, profile.name as profile_name FROM dt_members members, dt_profile profile WHERE profile.member_id=members.id $order";
		}
		else
		{
			...
			$query = "SELECT members.login as login, profile.id as id, profile.name as profile_name FROM dt_members members, dt_profile profile WHERE profile.member_id=members.id $order limit $from, $records_per_page";
		}
...
<a href="main.php?service=edit_profile.php&id=new"><img src="images/addnewmember.gif" border="0" align="top"></a>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<?php
	...
	{
?>
	<option value="<?php echo $k?>" <? if($sorting == $k) echo "selected"?>><?php echo $v ?>
<?php
	}
?>
</select>&nbsp;<input type=image src="images/sortlist.gif" border=0 alt="Sort List" align="top">
</form>
...
?>
   ...
   <tr>
          ...
          <td align="center" height="20" bgcolor="#F0F4EC" nowrap width="1%">&nbsp;&nbsp;
            <a href="javascript:deleteItem('<? echo $fProfile[ id ] ?>', 'all_profiles.php');"><img src="images/button_delete.gif" border="0"></a>
						 <a href=main.php?service=edit_profile.php&id=<? echo $fProfile[ id ] ?>><img src="images/button_edit.gif" border="0"></a>
						 <a href=preview_profile.php?id=<? echo $fProfile[ id ] ?> target=profile onClick="window.open('','profile','toolbars=0, scrollbars=1, resize=1')"><img src="images/button_view.gif" border="0"></a> &nbsp;&nbsp;</td>
        </tr>
