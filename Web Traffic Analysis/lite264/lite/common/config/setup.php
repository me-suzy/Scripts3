...
/*------------------------------------------------------------------------*/
  ...
  //get language
  if(isset($_POST['langlist'])) $this->lang=$_POST['langlist'];
  ...
  //step
  if(isset($_POST['extact_h'])) $step=$_POST['extact_h'];
  ...
  //check last version
  $flag=false;
  if(isset($_COOKIE['anver'])) {$flag=true;$ver=$_COOKIE['anver'];}
  elseif(isset($HTTP_COOKIE_VARS['anver'])) {$flag=true;$ver=$HTTP_COOKIE_VARS['anver'];}
  else {
    $checkver='http://www.actualscripts.com/scripts/versions/aalv.php?cv='.$this->version;
    $update='<script src="'.$checkver.'"></script>';
  }
  if($flag) {
  ...
  if(!strcmp($step,'step2')) {
    ...
    if($err->flag) {$err->reason('config.php|setup|diagnostics of file system has failed');$err->log_out();$err->flag=false;}
    $this->db_diagnostics($results);
    if($err->flag) {$err->reason('config.php|setup|diagnostics of database has failed');$err->log_out();$err->flag=false;}
    ...
    elseif(!strcmp($results[19],'Fail')) {tparse($c_fail,$vars);}
    else {tparse($c_skip,$vars);}
