...
function config($rf,$setupini=true) {
  global $err,$HTTP_POST_VARS,$_POST,$HTTP_GET_VARS,$_GET;

  $this->rf=$rf;                  //root folder of ActualAnalyzer
  $this->version=2.64;            //current version
  $this->name='ActualAnalyzer';   //Product name
  $this->series='Lite (free)';           //current series
  $this->url='./';                //url of the analyzer folder
  $this->site='http://www.actualscripts.com/products/analyzer/lite/';   //home page
  $this->folder='lite';           //default folder
  $this->dbhost='localhost:3306'; //host of database
  $this->dbase='analyzer';        //database name
  $this->dbuser='';               //user name (for database)
  $this->dbpass='';               //password (for database)
  $this->uname='admin';           //user name (for login)
  $this->passw='admin';           //password (for login)
  $this->vapass=1;                //authentication for view area
  ...
  $this->locktab=1;               //LOCK TABLES privilege 1=enabled
  ...
  //hours
  $hbtime=mktime($db['hours'],0,0,$db['mon'],$db['mday'],$db['year']);
  //time of the current hour
  $this->htime=mktime($dc['hours'],0,0,$dc['mon'],$dc['mday'],$dc['year']);
  ...
  $this->dtime=mktime(0,0,0,$dc['mon'],$dc['mday'],$dc['year']);
  //number of current day
  $this->dnum=round("%d",($this->dtime-$hbtime)/$this->time1);
  //number of the current month
  $this->mnum=($dc['year']-$db['year'])*12+$dc['mon']-$db['mon'];
  //begin time of the current month
  $this->mtime=mktime(0,0,0,$dc['mon'],1,$dc['year']);
  //begin time of the last month
  $this->lmtime=mktime(0,0,0,$dc['mon']-1,1,$dc['year']);
  //begin time of the last month
  $this->lpmtime=mktime(0,0,0,$dc['mon']-2,1,$dc['year']);
  //begin time of the next month
  $this->nmtime=mktime(0,0,0,$dc['mon']+1,1,$dc['year']);
  //begin time of the current week
  if($dc['wday']==0) $num=6;
  else $num=$dc['wday']-1;
  $this->wtime=mktime(0,0,0,$dc['mon'],$dc['mday']-$num,$dc['year']);
  //begin time of the last week
  $this->lwtime=mktime(0,0,0,$dc['mon'],$dc['mday']-7-$num,$dc['year']);
  //begin time of the next week
  $this->nwtime=mktime(0,0,0,$dc['mon'],$dc['mday']+7-$num,$dc['year']);

  //dynamic setting of language
  if(isset($_GET['language'])) $ilang=$_GET['language'];
  elseif(isset($_POST['language'])) $ilang=$_POST['language'];
  ...
  if(isset($_GET['style'])) $istyle=$_GET['style'];
  elseif(isset($_POST['style'])) $istyle=$_POST['style'];
...
//===================================================================
function setup() {
  global $err,$HTTP_POST_VARS,$_POST,$HTTP_COOKIE_VARS,$_COOKIE,$HTTP_SERVER_VARS,$_SERVER;

  require $this->rf.'common/config/setup.php';
}
...
function saveconf() {
  ...
  $fdata[]='locktab='.$this->locktab;
  if(file_exists($this->rf.'cdata.php')) {
    if(file_exists($this->rf.'cdata_bak.php')) {
      if(!unlink($this->rf.'cdata_bak.php')) {$err->reason('config.php|saveconf|can\'t delete file cdata_bak.php');return;}
    }
    if(!rename($this->rf.'cdata.php',$this->rf.'cdata_bak.php')) {$this->reason('config.php|saveconf|can\'t rename cdata.php to cdata_bak.php');return;}
  }
...
function db_diagnostics(&$test) {
  ...
  require $this->rf.'common/config/db_diag.php';
}
...
function fs_diagnostics(&$test) {
  ...
  require $this->rf.'common/config/fs_diag.php';
}
...
function crtables() {
  ...
  require $this->rf.'common/config/crtables.php';
}
