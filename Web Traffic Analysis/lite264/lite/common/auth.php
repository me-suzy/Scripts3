...
function auth($rf,$script,$title='',$pref='') {
  global $err,$conf,$HTTP_POST_VARS,$_POST,$HTTP_COOKIE_VARS,$_COOKIE;
  $vars = array();
  ...
  if(empty($uname)) {
    if(isset($_POST['unamef'])) $uname=$_POST['unamef'];
  ...
  if(empty($passw)) {
    if(isset($_POST['passwf'])) $passw=$_POST['passwf'];
    elseif(isset($HTTP_POST_VARS['passwf'])) $passw=$HTTP_POST_VARS['passwf'];
  }
  $remc='';
  if(isset($_COOKIE['aaauth'])) $remc=$_COOKIE['aaauth'];
  ...
  if(strcmp($uname,$conf->uname)||strcmp($passw,$conf->passw)) {
    ...
    //remember
    if(isset($_POST['authpan'])||isset($HTTP_POST_VARS['authpan'])) {
      if(isset($_POST['remlog'])||isset($HTTP_POST_VARS['remlog'])) $rem=$uname.'_'.$passw;
      else $rem='';
