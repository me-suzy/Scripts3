...
/*------------------------------------------------------------------------*/
  ...
  if($flag==0) {
      ...
      if(!$result) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      if(!mysql_num_rows($result)) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      $row=mysql_fetch_row($result);
      if(!$row[0]) {$err->reason('vdb.php|ref|\'vdb\' is busy -- '.mysql_error());return;}
  ...
  if($conf->locktab) {
    ...
    $result=mysql_query($request,$conf->link);
    if(!$result) {$err->reason('vdb.php|ref|blocking of tables has failed -- '.mysql_error());return;}
  }
  ...
  if(!$resultt) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}

  if($conf->locktab) {
    $request='UNLOCK TABLES';
    $resultu=mysql_query($request,$conf->link);
    if(!$resultu) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  }
  
  if($flag==0) {
      $request='DELETE FROM aa_tmp';
      $resultd=mysql_query($request,$conf->link);
      if(!$resultd) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}
      $request='SELECT RELEASE_LOCK("aa_lockv")';
      $reslock=mysql_query($request,$conf->link);
      if(!$reslock) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}
  ...
  if(!$result1) {$err->reason('vdb.php|ref|the request \''.$request.'\' has failed -- '.mysql_error());return;}
