...
<?
    ...
    // EDIT THIS IF YOU MODIFIED THE SINGUP PAGE OR
	...
	if ($AutoSend == 1)
	{
		...
		if ($add==1)
		{	
			// Loop through the whole $EmailerMessage to replace all occurrences of the keys
			// AVAILABLE KEYS: [[UNAME]], [[LNAME]], [[FNAME]], [[PASSWD]], [[EMAIL]], [[CONFIRM]], [[NEWPASS]]
			
			for ($loop = 0; $loop <= substr_count($EmailerMessage, "[[UNAME]]"); $loop++)
			{
				// Find the position of "[[UNAME]]" for uname 
				if (($UsernamePos = strpos($EmailerMessage, "[[UNAME]]")) !== false) {
					$EmailerMessage = substr_replace($EmailerMessage, $username, $UsernamePos, 9);
				}
			}
			
			for ($loop = 0; $loop <= substr_count($EmailerMessage, "[[FNAME]]"); $loop++)
			{
				// Find the position of "[[FNAME]]" for fname 
				if (($FnamePos = strpos($EmailerMessage, "[[FNAME]]")) !== false) 
				{
					$EmailerMessage = substr_replace($EmailerMessage, $fname, $FnamePos, 9);
				}
			}
			
			for ($loop = 0; $loop <= substr_count($EmailerMessage, "[[LNAME]]"); $loop++)
			{
				// Find the position of "[[LNAME]]" for lname 
				if (($LnamePos = strpos($EmailerMessage, "[[LNAME]]")) !== false) 
				{
					$EmailerMessage = substr_replace($EmailerMessage, $lname, $LnamePos, 9);
				}
			}
			
			for ($loop = 0; $loop <= substr_count($EmailerMessage, "[[PASSWD]]"); $loop++)
			{
				// Find the position of "[[PASSWD]]" for passwd 
				if (($PassPos = strpos($EmailerMessage, "[[PASSWD]]")) !== false) {
					$EmailerMessage = substr_replace($EmailerMessage, $password, $PassPos, 10);
				}
			}
			
			for ($loop = 0; $loop <= substr_count($EmailerMessage, "[[EMAIL]]"); $loop++)
			{
				// Find the position of "[[EMAIL]]" for email 
				if (($EmailPos = strpos($EmailerMessage, "[[EMAIL]]")) !== false) 
				{
					$EmailerMessage = substr_replace($EmailerMessage, $email, $EmailPos, 9);
				}
			}
			
			for ($loop = 0; $loop <= substr_count($EmailerMessage, "[[CONFIRM]]"); $loop++)
			{
				// Find the position of "[[CONFIRM]]" for confirm URL and confirmkey 
				if (($ConfirmPos = strpos($EmailerMessage, "[[CONFIRM]]")) !== false) 
				{
					$EmailerMessage = substr_replace($EmailerMessage, $confirm . '?confirmkey=' . $confirmkey, $ConfirmPos, 11);
				}
			}
